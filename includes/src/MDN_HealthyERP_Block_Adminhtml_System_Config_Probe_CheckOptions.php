...
{   
    ...
    private $_optionPathListToFix = null;
    private $_optionCountToFix = null;
    ...
    protected function checkProbe() {
      ...
      $this->_optionPathListToFix = self::getErrorsList();
      $this->_optionCountToFix = count($this->_optionPathListToFix);

      if($this->_optionCountToFix==0){
    ...
    protected function getCurrentSituation()
    {
      ...
      if (Mage::getStoreConfig('healthyerp/options/display_basic_message')){
        $situation = $this->__('Options with uncorrect settings').' : '.$this->_optionCountToFix.'<br/>';
        ...
        switch($this->_indicator_status){
          ...
          case parent::STATUS_PARTIAL :
             if($this->_optionCountToFix>0){
              ...
              foreach ($this->_optionPathListToFix as $id) {
