    ...
    public function getShippingRatesCollection()
    {
        if (is_null($this->_rates)) {
            $this->_rates = Mage::getModel('sales/quote_address_rate')->getCollection()
                ->setAddressFilter($this->getId())->setOrder('price',ASC);
                //echo "<pre>"; print_r($this->_rates); exit;
               //echo  $this->_rates->load(1); exit;
    ...
    public function getGroupedAllShippingRates()
    {
        ...
        foreach ($this->getShippingRatesCollection() as $rate) {
            if (!$rate->isDeleted() && $rate->getCarrierInstance()) {
                ...
                $rates[$rate->getCarrier()][0]->carrier_sort_order = $rate->getPrice();
            }
        }
