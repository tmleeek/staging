...
class Tatva_Cibleweb_Model_Exportxml1 extends Mage_Core_Model_Abstract {
...
public function xml_escape($s)
{
	...
	public function saveCsvFileLocal( $datas ){
		...
		if( ! $this->createDir( $root )){
		 ...
		 $pathFile = $_SERVER['DOCUMENT_ROOT'].'lengow/'.'lengow1.xml';
         try{
            ...
            //foreach ($productCollection as $product){
                try{
                    //  Mage::log($product->getEntityId(),null,"lengow.log");
                     ...
                     $prod = Mage::getModel('catalog/product')->load(7775);   // echo $id.'<br>';
                     $deee=$prod->getDeee();
