	...
	protected function insertTvaCols(&$page) {
		...
		foreach ( $this->invoice->getAllItems () as $item ) {
		 ...
		 // echo "<pre>"; print_r($item->getData()); exit;
            ...
            $weee_amt = $weee_amt * $orderItem->getQtyOrdered();
            $item_amt = $orderItem->getRowTotal() - $weee_amt;
            //$tvaDetail [$codeTva] ['sstotht'] = $tvaDetail [$codeTva] ['sstotht'] + $tax_info[0]['base_amount'];
            //$tvaDetail [$codeTva] ['tva'] = $tvaDetail [$codeTva] ['tva'] +$tax_info[0]['base_amount'];
			/*$tvaDetail [$codeTva] ['totht'] = $tvaDetail [$codeTva] ['totht'] + $item->getBaseRowTotal ()  - $orderItem->getBaseDiscountAmount ()+ $this->invoice->getShippingAmount () +$weee_amt_without_tax; */

	       $tvaDetail [$codeTva] ['totht'] = $tvaDetail [$codeTva] ['totht'] + $orderItem->getBaseRowTotal ()  - $orderItem->getBaseDiscountAmount ()+ $this->order->getShippingAmount () +$weee_amt_without_tax;
	...
	protected function addItem($page, $item) {//echo '<pre>';print_r($item->getData());exit;
		...
		if ($options) {
			...
			foreach ( $options as $option ) {
             ...
             $weee_amt = Mage::helper('weee')->getAmountForDisplayInclTaxes($_product);
             $final_bas_price = $item->getPrice()+$item->getTaxAmount() + $weee_amt;
             $item_price = number_format($final_bas_price,2);

          	 $item_row_total_temp = number_format($item->getBaseRowTotal(),2) + $item->getTaxAmount() +$weee_amt;
          	 $item_row_total = number_format($item_row_total_temp,2);


		// Tax
		$this->_setRedContentFont ( $page );
		$string = number_format ( $item_price, 2 );
		$pos = $this->getAlignCenter ( $string, 332, 385 - 332, $page->getFont (), $page->getFontSize () );
		$page->drawText ( $string, $pos, $baseY, 'UTF-8' );
		// Unit price
		$string = Mage::helper ( 'core' )->formatPrice ( $weee_amt, false );
		...
		$page->drawText ( $string, $pos, $baseY, 'UTF-8' );
		// total ht
		$string = Mage::helper ( 'core' )->formatPrice ( $item_row_total, false );
