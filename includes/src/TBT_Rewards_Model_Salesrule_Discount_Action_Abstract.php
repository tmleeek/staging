...
abstract class TBT_Rewards_Model_Salesrule_Discount_Action_Abstract extends Mage_Core_Model_Abstract
{
    ...
    protected function _getDiscountableRowTotal($address, $item, $rule)
    {
        ...
        // Calculate the row totals. Catalog Discount already subtracted in TBT_Rewards_Model_Sales_Quote_Address_Total_Rewards::getFinalPrice()
        $item_row_total = $itemPrice * $qty;
        $item_base_row_total = $baseItemPrice * $qty;
        ...
        if (!Mage::helper('rewards/version')->isBaseMageVersionAtLeast('1.4.2.0')) {
            ...
            $item_row_total += $catalog_discount;
            $item_base_row_total += $base_catalog_discount;
        }
