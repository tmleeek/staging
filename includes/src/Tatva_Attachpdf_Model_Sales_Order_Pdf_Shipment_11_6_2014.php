...
Deny from all';
	...
	protected function insertHeaderItems(&$page) {
		...
		$page->drawText ( $string, $pos, $y, 'UTF-8' );
		// Options
		$string = Mage::helper ( 'tatvasales' )->__ ( 'Options' );
		...
		$page->drawText ( $string, $pos, $y, 'UTF-8' );
		// Unit price
		$string = Mage::helper ( 'tatvasales' )->__ ( 'Unit Price (VAT excl.)' );
		...
		$page->drawText ( $string, $pos, $y, 'UTF-8' );
		// total ht
		$string = Mage::helper ( 'tatvasales' )->__ ( 'Total (VAT excl.)' );
   ...
   /**
	...
	protected function addItem($page, $item) {
		...
		$string = false;
            ...
            $string = $prd->getGammeCollection();
			
            //$string .= $prd->getMarque();
            //$brand = $this->getBrand($prd->getMarque());
            ...
            {
            $p = $prd->getMarque(); 
			$attr = $prd->getResource()->getAttribute("marque");
		...
		// Options
		$options = $this->getItemOptions ( $item->getOrderItem() );
		if ($options) {
			$_height = 7;
			$_y = $this->y - $_height;
			switch (count ( $options )) {
		...
		$page->drawText ( $string, $pos, $baseY, 'UTF-8' );
		// Unit price
		$string = Mage::helper ( 'core' )->formatPrice ( $item->getPrice (), false );
		$pos = $this->getAlignCenter ( $string, 385, 456 - 385, $page->getFont (), $page->getFontSize () );
		$page->drawText ( $string, $pos, $baseY, 'UTF-8' );
		// qty
		...
		$page->drawText ( $string, $pos, $baseY, 'UTF-8' );
		// total ht
		$string = Mage::helper ( 'core' )->formatPrice ( $item->getPrice () * $item->getQty (), false );
		$pos = $this->getAlignCenter ( $string, 489, 561 - 489, $page->getFont (), $page->getFontSize () );
		$page->drawText ( $string, $pos, $baseY, 'UTF-8' );
	...
	public function getPdf_TNT($shipments = array()) {
		...
		foreach ( $shipments as $shipment ) {
           ...
           $orderNum = Mage::getModel('sales/order_shipment')->load($shipment->getID())->getOrder()->getRealOrderId();
            ...
            $pdf_merge->addPDF($path.$orderNum.'.pdf','all')->addPDF($path.'packingslip_'.$shipment->getId().'.pdf','all');
