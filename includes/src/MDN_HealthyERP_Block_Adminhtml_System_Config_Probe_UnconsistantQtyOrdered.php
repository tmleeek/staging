...
HAVING
    ...
    protected function getCurrentSituation()
    {
      ...
      if (Mage::getStoreConfig('healthyerp/options/display_basic_message')){
        $situation = $this->__('Product which the ordered qty is not consistant').' : '.$this->_productCountToFix.'<br/>';
             ...
             if (Mage::getStoreConfig('healthyerp/options/display_advanced_message')){
              if($this->_productCountToFix>0){
                 ...
                 foreach ($this->_productIdListToFix as $diffItem) {
                    $situation .= "Product#".$diffItem['pid']." orderedQtyStored=".$diffItem['qty']." in warehouseId=".$diffItem['stock_id']." orderedQtyExpected=".$diffItem['expected']."<br/>";
                 }
              }
             }
            break;
           ...
           foreach ($productListToFix as $diffItem) {
               ...
               if($productId>0 && $expectedQty>=0 && $warehouseId>=0 && intval($qty) != intval($expectedQty) ){
                 $helper->AddTask('Refresh Stock Ordered Qty for product#'.$productId,
                                'AdvancedStock/Product_Ordered', 'updateStockOrderedQty', $diffItem,
                                $taskGroupCode,
                                false,
                                $priority);
              }
