<?php
class Tatva_Cibleweb_Model_Exportchfxml extends Mage_Core_Model_Abstract
{
    ...
    {
        if (!$this->_storeModel) {
            $this->_storeModel = Mage::getModel('core/store')->load('chfr');
        } //!$this->_storeModel
    ...
    public function saveCsvFileLocal($datas)
    {
        ...
        $pathFile = $_SERVER['DOCUMENT_ROOT'] . 'lengow/' . 'lengow_chf.xml';
        try
        {
            ...
            $pathFile = $_SERVER['DOCUMENT_ROOT'] . '/lengow/' . 'lengow_chf.xml';
            $pathFile = Mage::getBaseDir() . DS . 'lengow' . DS . 'lengow_chf.xml';
        ...
        for ($pageNumber = 1; $pageNumber <= $lastPageNumber; $pageNumber++)
        {
            ...
            foreach ($productCollection as $product)
            {
                try
                {
                    ...
                    $prod    = Mage::getModel('catalog/product')->setStoreId(12)->load($id); // echo $id.'<br>';
                    ...
                    if ($this->getCategoriesName($product))
                    {
                        ...
                        foreach ($ids as $id)
                        {
                            $name = Mage::getModel('catalog/category')->setStoreId(12)->load($id)->getName();
                            if ($id != 2)
                            {
                                $cat_name .= $name . ' > ';
                            }
    ...
    public function getselectAttributevalue($attr, $att_ids)
    {
        ...
        foreach ($att_array as $att_array_id)
        {
            ...
            foreach ($attribute->setStoreId(12)->getSource()->getAllOptions(false) as $option)
            {
    ...
    public function getShippingAmount($weight, $tax)
    {
        ...
        if (Mage::getStoreConfig('carriers/colissimo/active') == 1)
        {
            ...
            $rule         = Mage::getModel('tatvashipping/rule')->getCollection()->addShippingFilter($shipping->getCode())->addCountryFilter($countryId)->addWeightFilter($weight)->getFirstItem();
            if ($rule && $rule->getId())
            {
    ...
    public function getCategoryName($product)
    {
        ...
        $categorys = Mage::getSingleton('catalog/category')->getCollection()->setStoreId(12)->addAttributeToSelect('name')->addAttributeToFilter('entity_id', array(
            'in' => $catIds
        ))->addAttributeToFilter('level', 4);
        if ($categorys->getSize())
        {
            $id = $categorys->getFirstItem()->getEntityId();
            return Mage::getModel('catalog/category')->setStoreId(12)->load($id)->getName();
        }
        else
        {
            $categorys = Mage::getSingleton('catalog/category')->getCollection()->setStoreId(12)->addAttributeToSelect('name')->addAttributeToFilter('entity_id', array(
    ...
    public function getPromotion($product)
    {
        $promo               = array();
        $customerGroupId     = 0;
        $catalogRuleProducts = Mage::getModel('catalogrule/rule_product_price')->getCollection()->addFieldToFilter('main_table.website_id', '10')->addFieldToFilter('main_table.customer_group_id', $customerGroupId);
        ...
        ), 'rule_id')->where('rule_product.product_id = main_table.product_id ')->where('rule_product.customer_group_id = ?', $customerGroupId)->where('rule_product.website_id = ?', '10');
