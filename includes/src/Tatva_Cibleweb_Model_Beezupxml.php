<?php
class Tatva_Cibleweb_Model_Beezupxml extends Mage_Core_Model_Abstract
{
    ...
    public function saveCsvFileLocal($datas)
    {
        ...
        try
        {
            ...
            $pathFile = $_SERVER['DOCUMENT_ROOT'] . '/beezup/' . 'beezup.xml';
            $pathFile = Mage::getBaseDir() . DS . 'beezup' . DS . 'beezup.xml';
        ...
        for ($pageNumber = 1; $pageNumber <= $lastPageNumber; $pageNumber++)
        {
            ...
            foreach ($productCollection as $product)
            {
                try
                {
                    ...
                    $prod    = Mage::getModel('catalog/product')->setStoreId(3)->load($id); // echo $id.'<br>';
                    ...
                    if ($promo)
                    {
                        ...
                        if ($line['discount_percentage'] > 0)
                        {
                            ...
                            $line['prix'] = number_format($newPrice, 2);
    ...
    public function getselectAttributevalue($attr, $att_ids)
    {
        ...
        foreach ($att_array as $att_array_id)
        {
            ...
            foreach ($attribute->setStoreId(3)->getSource()->getAllOptions(false) as $option)
            {
    ...
    public function getProductCollection($filter = false, $pageNumber = NULL)
    {
        ...
        $collection->setStoreId(3);
        $collection->addAttributeToFilter('status', '1');
        //$collection->addFieldToFilter ('entity_id',4654);
    ...
    public function getCategoryName($product)
    {
        ...
        $categorys = Mage::getSingleton('catalog/category')->getCollection()->setStoreId(3)->addAttributeToSelect('name')->addAttributeToFilter('entity_id', array(
            'in' => $catIds
    ...
    public function getPromotion($product)
    {
        $promo               = array();
        $customerGroupId     = 0;
        $catalogRuleProducts = Mage::getModel('catalogrule/rule_product_price')->getCollection()->addFieldToFilter('main_table.website_id', '2')->addFieldToFilter('main_table.customer_group_id', $customerGroupId);
        ...
        ), 'rule_id')->where('rule_product.product_id = main_table.product_id ')->where('rule_product.customer_group_id = ?', $customerGroupId)->where('rule_product.website_id = ?', '2');
