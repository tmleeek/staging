...
class Quadra_Cybermutforeign_Model_Observer
{
    /**
     *  Can redirect to Cybermutforeign payment
     */
    ...
    public function multishippingRedirectUrl(Varien_Event_Observer $observer)
    {
        if (Mage::getSingleton('checkout/session')->getCanRedirect()) {
            ...
            if (!(strpos($order->getPayment()->getMethod(), 'cybermutforeign') === false)) {
                Mage::getSingleton('checkout/session')->setRealOrderIds(implode(',', $orderIds));
                Mage::app()->getResponse()->setRedirect(Mage::getUrl('cybermutforeign/payment/redirect'));
            }
        } else {
