...
$file = Mage::getBaseDir() . '/tatvascript/export_products_simple_material.csv';
      ...
      while (($data = fgetcsv($handle, 1000, ',', '"')) !== FALSE) {
            ...
            // 211 is color attribute_id
  			// Load by SKU
                  ...
                  {
                     $str_new='';
                     $str_new = str_replace("'",'"',$data[1]);
                     $data_sql="SELECT DISTINCT material FROM `tatva_material_temp` WHERE `old_material` ='".$str_new."'";
                     ...
                     if($ids!='' && $data[1]!='')
                     {
                       echo $sql = "INSERT INTO ".$cvarchartable." (entity_type_id, attribute_id, store_id, entity_id, value)
    		                                          VALUES ('4', '212', '".$storeId."', '".$product_id."', '".$ids."')";   echo '<br>--------------<br>';
                       $write->query($sql);
                       $row++;
                     }
                  }
