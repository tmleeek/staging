...
Mage::log("customer import",null,"order_import_log.log");
...
Mage::log("products import",null,"order_import_log.log");
        ...
        foreach ($products_ordered as $data) {
            ...
            if (!$product || !$product->getId()) { echo 'yes=='.$product_id.'=='.$importData['order_id'];exit;
				...
				// add by nisha
				 if($parts[2] != 'bundle')
				{
				   //$product = $this->createProductnew($parts, $importData['store_id']);
				   
				     $File = $_SERVER['DOCUMENT_ROOT']."/order_not_import_simple.txt";
				     $Handle = fopen($File, 'a'); 
					 
					 $Data = "simple product not existe so order not improted==".$parts[0]."==".$importData['order_id']."\r\n";
					 fwrite($Handle, $Data);
					 fclose($Handle);
					 return; 			   
				...
				else
				{
				     ...
				     //added by nisha
					 $File = $_SERVER['DOCUMENT_ROOT']."/order_not_import.txt";
				     $Handle = fopen($File, 'a'); 
					 
					 $Data = "bundle product not existe so order not improted==".$parts[0]."==".$importData['order_id']."\r\n";
					 fwrite($Handle, $Data);
					 fclose($Handle);
					 return;  
				} 
...
Mage::log("test1",null,"order_import_log.log");
	    try {
...
Mage::log("test2",null,"order_import_log.log");
        ...
        //process order
        if (!empty($orderData)) {
            $this->_initSession($orderData['session']);Mage::log("test3",null,"order_import_log.log");
	...
	public function createProductnew($parts,$store_id) {
        ...
        if(!$product) {
			...
			$product = new Mage_Catalog_Model_Product(); 
					...
					//$classTax = Mage::getModel('tax/class')->setClassType(Mage_Tax_Model_Class::TAX_CLASS_TYPE_PRODUCT)->load('TVA 19,6%','class_name');
    ...
    {Mage::log("test4",null,"order_import_log.log");
        ...
        try {
			...
			try {Mage::log("test5",null,"order_import_log.log");
...

    {
        ...
        if (method_exists($product, 'getTypeId')) {
           ...
           /*nisha if ($productstockItem->getStockItem()->getIsInStock() && $productstockItem->getStockItem()->getManageStock() == 1 && $stockItem->getQty() != "0.0000" || $productstockItem->getStockItem()->getManageStock() == 0) {*/
...
//modify by nisha
 ...
 //echo '<pre>';print_r($add_products_array);exit;
    }
		 ...
		 if (method_exists($product2, 'getTypeId')) {
			...
			try {
				...
				if($productexistsletsnotupdateprice != true) {
					/*echo "UPDATE `" . $prefix . "sales_flat_order_item` SET price_incl_tax = '" . $parts[2] . "', base_price_incl_tax = '" . $parts[2] . "', base_row_total_incl_tax = '" . $custom_row_total . "', row_total_incl_tax = '" . $custom_row_total . "', row_total = '" . $order_total_without_tax . "', base_row_total = '" . $order_total_without_tax . "', tax_amount = '" . $tax_percent_for_row_total . "', base_tax_amount = '" . $tax_percent_for_row_total . "', original_price = '" . $parts[2] . "', base_original_price = '" . $parts[2] . "', price = '" . $parts[2] . "', base_price = '" . $parts[2] . "' WHERE order_id = '" . $order1->getId() . "' AND sku = '" . $productskutocheck . "'";exit;*/
    ...
    {//echo 'hello';exit;
        ...
        if (isset($parts[3]) && $parts[2] != "configurable" && $parts[2] != "bundle" && $parts[2] != "simple") {
            ...
            try {
				echo "UPDATE `" . $prefix . "sales_flat_order_item` SET name = \"" . addslashes($customproductname) . "\", sku = \"" . $customproductsku . "\" WHERE item_id = '" . $db_item_id . "'";
