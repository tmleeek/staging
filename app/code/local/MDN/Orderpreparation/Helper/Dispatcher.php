...
class MDN_Orderpreparation_Helper_Dispatcher extends Mage_Core_Helper_Abstract {
    ...
    public function DispatchOrder($order) {
        ...
        if (($order->getstate() == 'complete') || ($order->getstate() == 'canceled'))
        {
        ...
        foreach ($warehouses as $warehouse) {
            ...
            if (!$this->orderBelongsToSelectedOrders($order, $warehouse)) {
                if (!$order->IsCompletelyShipped()) {
					...
					//APP CODE LOCAL CHANGE					
                    if($opp_type == 'stockless')
                        $this->RecalculatesQtysAndTryToReserve($order,$warehouse->getId());                    
                    //APP CODE LOCAL CHANGE	
	...
	//APP CODE LOCAL CHANGE	
	public function RecalculatesQtysAndTryToReserve($order, $warehouseId = null)
    {
        foreach ($order->getItemsCollection() as $item) {
            if (($warehouseId != null) && ($warehouseId != $item->getpreparation_warehouse()))
                continue;            
            mage::helper('AdvancedStock/Product_Base')->planUpdateStocksWithBackgroundTask($item->getproduct_id(), 'from order is stock less');
        }
    }
	//APP CODE LOCAL CHANGE	
