...
class Autocompleteplus_Autosuggest_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getConfigMultiScopesDataByFullPath($path)
    {
        ...
        if (!$rows = Mage::getSingleton('core/config_data')->getCollection()->getItemsByColumnValue('path', $path)) {
            ...
            foreach ($rows as $row) {
                $values[$row->getScopeId().'_'.$row->getScope()] = $row->getValue();
            }
        }
...
//                         'stores'  => array(array_pop($storesArr)),
            ...
            foreach ($storesArr as $key => $value) {
                ...
                if (array_key_exists($key.'_stores', $locales)) {
                    $storeComplete['lang'] = $locales[$key.'_stores'];
                } elseif(array_key_exists($storeComplete['website_id'].'_websites', $locales)) {
                    $storeComplete['lang'] = $locales[$storeComplete['website_id'].'_websites'];
                }else{
                   $storeComplete['lang'] = $locales['0_default'];
                }

                if (array_key_exists($key.'_stores', $storeUrls)) {
                    $storeComplete['url'] = $storeUrls[$key.'_stores'];
                } elseif(array_key_exists($storeComplete['website_id'].'_websites', $storeUrls)) {
                    $storeComplete['url'] = $storeUrls[$storeComplete['website_id'].'_websites'];
                }else{
                   $storeComplete['url'] = $storeUrls['0_default'];
                }

                if ($useStoreCode) {
                    $storeComplete['url'] = $storeUrls['0_default'].$value['code'];
                }
