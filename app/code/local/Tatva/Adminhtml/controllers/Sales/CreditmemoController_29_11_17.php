   ...
   //create by nisha for export account accuracy
    public function exportaccountAction()
    {
        ...
        for($i=0;$i<count($creditmemoIds);$i++)
        {
            ...
            //echo "<pre>";print_r($order->getData());print_r($creditmemo->getData());die();
            $total = array();
            $from = $order->getOrderCurrencyCode();
            //$from = $order->getGlobalCurrencyCode();
            $store_data = Mage::getModel('core/store')->load($order->getStoreId());
            $orderSotreName = $store_data->getName();               //$from == "USD"  // $from == "CHF"  //$from == "EUR"  //$from == "GBP"
            ...
            $baseamount_refnded = $order->getGrandTotal();
            $currency_data = '';
            //exit;
            /*if($from == "USD")
            {
            ...
            if ($creditmemo->getShippingAmount () > 0)
            {
                ...
                if($from=="GBP")
                {
                    //$tvaDetail [$codeTva] ['port'] = $creditmemo->getBaseShippingAmount(); // changed due to conversion
                ...
                $tvaDetail [$codeTva] ['dee'] = $weee_amt * $item->getQty();
                $tax = $creditmemo->getShippingAmount () * $orderItem->getTaxPercent () / 100;
            ...
            foreach ($tvaDetail as $tvaItem )
            {
                ...
                {
                    $value =  '"'.$creditmemoDate .'","'.$orderSotreName.'","VE","0","'.$code_vat.'","'.$customer_name.'","'.$orderCountryName.'","","'.number_format($creditmemo->getTaxAmount(),2,'.','').'","'.$from.'","'.$currency_data.'","'.$creditmemo->getIncrementId().'","'.$order_inc_id.'"';
                    @fwrite($fp, $value."\n");
                }
