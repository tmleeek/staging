    ...
    public function saveAction()
    {
        ...
        try
        {
            if ($shipment = $this->_initShipment())
            {
                ...
                $order_data = Mage::getModel('sales/order')->load($shipment['order_id']);
                $ekomi_store_id = $order_data->getStoreId();

                $ship_time_by_erp= $planning['psop_shipping_date'];
                //echo strtotime($cuurent_time)."<=".strtotime($ship_time_by_erp);die();
                ...
                if ($_shippingMethod[0] == 'pointsrelais')
                {
                    ...
                    $adress = $_order->getBillingAddress()->getStreet();
                    ...
                    {
                    	$shipping_mob = $cust_shipping_mobile;
                    }
                    else
                    {
                    	$shipping_mob = $_order->getShippingAddress()->getTelephone();
                    }

                    //echo '<pre>';
                    //print_r($shipping_mob);exit;
                    ...
                    $params = array(
                                   ...
                                   'ModeLiv'        => '24R',
                                   'Expe_Langage'   => 'FR',
                                   'Expe_Ad1'       => trim($this->removeaccents($_order->getShippingAddress()->getFirstname() . ' ' . $_order->getShippingAddress()->getLastname())),
                                   'Expe_Ad3'       => trim($this->removeaccents($adress[0])),
                                   'Expe_Ad4'       => trim($this->removeaccents($adress[1])),
                                   'Expe_Ville'     => trim($this->removeaccents($_order->getShippingAddress()->getCity())),
                                   'Expe_CP'        => $_order->getShippingAddress()->getPostcode(),
								   'Expe_Pays'      => trim($_order->getShippingAddress()->getCountryId()),
                    ...
                    //echo "<pre>"; print_r($expedition); exit;
