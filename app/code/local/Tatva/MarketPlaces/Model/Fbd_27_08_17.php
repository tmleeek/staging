...
class Tatva_MarketPlaces_Model_Fbd extends Tatva_MarketPlaces_Model_Abstract
{
                    ...
                    if(!empty($orders))
                    {
                        try
                        {
                            ...
                            //exit;
                                ...
                                $this->addShippingMethod(utf8_encode((string)$shipping_method));
                    		...
                    		foreach($orders->items->item as $item)
                            {
                              ...
                              $product_id = $product->getEntityId();
                  			  ...
                  			  if($product_id ==0)
                  			  {
                  			  	   Mage::log ( 'The product does not exist' , Zend_log::INFO, $this->getLogFile() );
                              ...
                              exit;*/
                    			...
                    			Mage::log ( 'FIN Product Processing' , Zend_log::INFO, $this->getLogFile() );
                    		}
            ...
            if (!empty($files))
            {
                foreach ($files as $file)
                {
                    if ($file != '.' && $file != '..' && $file != '_archive')
                    {
                        ...
                        $org_remote_source = 'ssh2.sftp://'.$resSFTP.$remoteDir.'/'.$file;
    ...
    protected function addShippingMethod($shippingMethod)
    {
		...
		$shipping = array('Colissimo' => 'colissimo','TNT Express' => 'tnt');
        ...
        {
            $carrierModel = "tatvashipping/carrier_colissimo";
