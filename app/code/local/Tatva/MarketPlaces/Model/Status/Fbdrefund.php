 ...
 * @package Tatva_MarketPlaces
 */
class Tatva_MarketPlaces_Model_Status_Fbdrefund extends Mage_Core_Model_Abstract
{
	...
	public function execute()
    {
		...
		Mage::log ( 'FBD - Mis à jours des status chez FBD: DEBUT', Zend_log::INFO, $this->_fbdModel()->getLogFile() );
        ...
        {
			Mage::log ( 'FBD - Mis à jours des status chez FBD: AUCUNE CMD A ENVOYER', Zend_log::INFO, $this->_fbdModel()->getLogFile() );
			return;
		}
    		    ...
    		    if( $order->getStatus() == self::STATUS_COMPLETE || $order->getStatus() == self::STATUS_INVOICE)
                {
                    ...
                    foreach($order_items as $order_item)
                    {
                        ...
                        $shipment_query = "SELECT
                                                ...
                                                creditmemo_item.qty as creditmemo_qty,
                                                creditmemo.increment_id as creditmemo_inc_id,
                                                creditmemo.updated_at as creditmemo_updated_at,
                                                creditmemo_item.price as creditmemo_product_price
                                            ...
                                            INNER JOIN
                                                sales_flat_creditmemo as creditmemo
                                                ON invoice.order_id=creditmemo.order_id
                                            INNER JOIN
                                                sales_flat_creditmemo_item as creditmemo_item
                                                ON creditmemo.entity_id=creditmemo_item.parent_id
                                                and shipment_item.order_item_id=".$order_item->getItemId()."
                                                and invoice_item.order_item_id=".$order_item->getItemId()."
                                                and creditmemo_item.order_item_id=".$order_item->getItemId();
                        ...
                        if(!empty($shipment_results))
                        {
                            ...
                            for ($x = $ac; $x <= $shipment_results['shipment_qty']; $x++)
                            {
                               $sequence = $dom->createElement ( "sequence" );
                                ...
                                $pdf_filename = $dom->createElement ("pdf_filename", $shipment_results['invoice_pdf_file']);
                            ...
                            $al = 1;
                            for ($x = $al; $x <= $shipment_results['shipment_qty']; $x++)
                            {
                               $sequence = $dom->createElement ( "sequence" );
                                ...
                                // Append Ordered Product Status
                                $seq_state = $dom->createElement ("state", "refunded");
                                $sequence->appendChild( $seq_state );

                                $item_refunded = $order_item->getQtyRefunded();
                                if($item_refunded > 0)
                                {
                                    if($shipment_results['creditmemo_qty'] > 0)
                                    {
                                         $refund = $dom->createElement ( "refund" );

                                        // Append Invoice Id
                                        $creditmemo_id = $dom->createElement ("id", $shipment_results['creditmemo_inc_id']);
                                        $refund->appendChild($creditmemo_id);

                                        // Append Invoice created at
                                        $creditmemo_at = $dom->createElement ("refunded_at", $shipment_results['creditmemo_updated_at']);
                                        $refund->appendChild($creditmemo_at);

                                        // Append Invoice amount
                                        $creditmemo_item_price = $dom->createElement ("amount", $shipment_results['creditmemo_product_price']);
                                        $refund->appendChild($creditmemo_item_price);

                                        // Append Invoice pdf_filename
                                        $refund_shipping = $dom->createElement ("refund_shipping", "no");
                                        $refund->appendChild( $refund_shipping );

                                        $sequence->appendChild( $refund );
                                    }
                                }
                            ...
                            $item_canceled = $order_item->getQtyCanceled();
                            if($item_canceled > 0)
                            {
                                $ax = 1;
                                for ($y = $ax; $y <= $item_canceled; $y++)
                                {
                                    $sequence = $dom->createElement ( "sequence" );
                    ...
                    $order->setMarketplacesOrderSended ("R");
                        
				    $order->getResource()->saveAttribute ( $order, 'marketplaces_order_sended' );
                    $created_date = date('_Ymd_his');

                    $resFile = fopen("ssh2.sftp://{$resSFTP}/to_fbd/order_invoice/az-boutique_update_order_".$Fbd_order.$created_date.".xml", 'w');
                    fwrite($resFile,$xml);
    		    }
		...
		Mage::log ( 'FBD - Mis à jours des status chez FBD: FIN', Zend_log::INFO, $this->_fbdModel()->getLogFile() );
	}
	...
	protected function _getOrderCollection()
    {
		if(!$this->_collectionOrders)
        {
			...
			$this->_collectionOrders = Mage::getModel('sales/order')->getCollection()
							...
							->addAttributeToFilter( 'marketplaces_order_sended', array('in'=>'O'))
                            ->addAttributeToFilter( 'total_refunded', array('gt'=>'0'))
						    ->addAttributeToFilter( 'marketplaces_partner_code', array('in'=>$fbdCode)) ;
		}
            $this->_collectionOrders->getSelect();
