<?php
class Tatva_Exportaccountancy_Model_Exportdata extends Mage_Core_Model_Abstract
{
    public function exportaccountancy()
    {
        /* Format our dates */
        ...
        //$first_date = date('Y-m-d',strtotime('first day of january'));
        //$last_date = date('Y-m-d',strtotime('first day of february'));

        //$last_month = "Jan";

        $path = '/home/azboutik/public_html/exportaccounting/';
        ...
        foreach($invoices as $invoice)
        {
            ...
            //echo "<pre>";print_r($order->getData());print_r($invoice->getData());die();
            $total = array();
            ...
            //$from == "USD"  // $from == "CHF"  //$from == "EUR"  //$from == "GBP"
            ...
            //$payment_data = $payment->getMethod();
            ...
            foreach ($tvaDetail as $tvaItem )
            {
                ...
                {
                    $value =  '"'.$invoiceDate .'","'.$orderSotreName.'","VE","0","'.$code_vat.'","'.$customer_name.'","'.$orderCountryName.'","","'.number_format($invoice->getTaxAmount(),2,'.','').'","'.$from.'","'.$currency_data.'","'.$invoice->getIncrementId().'","'.$order_inc_id.'"';
                    @fwrite($fp, $value."\n");
                }
