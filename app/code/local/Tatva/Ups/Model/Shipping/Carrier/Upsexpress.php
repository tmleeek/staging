 ...
 */
class Tatva_Ups_Model_Shipping_Carrier_Upsexpress
     extends Tatva_Usa_Model_Shipping_Carrier_Ups
    implements Mage_Shipping_Model_Carrier_Interface

{

    protected $_code = 'upsexpress';
    ...
    { //echo 'hello=='.Mage::getStoreConfig('carriers/upsexpress/allowed_methods');
        ...
        if ($request->getDestCountryId()) {
            $destCountry = $request->getDestCountryId();
                ...
                if($request->getDestRegionId() != '' || $request->getDestRegionId() != 0)
                {
                    ...
                    $special_region_code = array("GP", "MQ", "GF","RE", "PM", "YT", "TF","WF", "PF", "NC", "MC", "YY", "MF", "BL");

                    if (in_array($regionModel_code, $special_region_code) && $destCountry == "FR")
                    {
                        $destCountry = $regionModel_code;
                    }
	...
	public function getDiscountAmount(){
	    ...
	    if (in_array($country_id, $var_gls_arr)) 
		{
			...
			if($gls->getConfigData('free_shipping_enable') == true && $this->getConfigData('deduct_colissimo_amount') == 1 ){
				$seuil = $gls->getFreeShippingSubtotal($country_id); //echo $request->getPackageValue() .'>='. $seuil;
        ...
        if (empty($priceArr)) {
            ...
            foreach ($priceArr as $method=>$price) {
                ...
                if($costArr[$method] > 0){
                	...
                	if($new_cost > $this->getConfigData('colissimo_deduct_price'))
                		$rate->setCost($new_cost);
					else
						$rate->setCost($this->getConfigData('colissimo_deduct_price'));
                }else{
                 ...
                 if($price > 0){
                	...
                	if($new_price > $this->getConfigData('colissimo_deduct_price'))
                		$rate->setPrice($new_price);
					else
						$rate->setPrice($this->getConfigData('colissimo_deduct_price'));
                }else {
                	$rate->setPrice($price);
                }
				//$rate->setAmountBeforeDiscount($costArr[$method]);
                $rate->setAmountBeforeDiscount($price);
                $result->append($rate);
            }
        }
    ...
    { 
        ...
        if (strlen(trim($xmlResponse))>0) {
            ...
            if($success===1){ 
                ...
                $allowedMethods = explode(",", $this->getConfigData('allowed_methods'));
        //echo '<pre>';print_r($allowedMethods);
                // Negotiated rates
...
//echo '<pre>';print_r($arr);
                foreach ($arr as $shipElement){
					...
					//echo 'code=='.$code;
					$ship_code = $this->getShipmentByCode($code);
                    #$shipment = $this->getShipmentByCode($code);
                    //comment by nisha if (in_array($code, $allowedMethods)) {
                   
                   	if ($ship_code == 'UPS Express') {                   	
                        if ($negotiatedActive) {
...
XMLAuth;
    ...
    protected function _parseXmlTrackingResponse($trackingvalue, $xmlResponse)
    {
        ...
        if ($resultArr) {
            ...
            $tracking->setCarrier('upsexpress');
