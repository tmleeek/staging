            ...
            )
        {
            ...
            foreach ($_option->getValues() as $_value)
            {
                ...
                $collection = Mage::getModel('catalog/product')
                    ->getCollection()
                    ->addAttributeToFilter('sku', $_value->getSku())
                    //->addAttributeToFilter('sku', array('in' => $_value->getSku()))
                    //->addIdFilter($stockIds)
                    ->addAttributeToSelect($attributes);*/


                $_product = Mage::getModel('catalog/product')->loadByAttribute('sku', $_value->getSku());
                ...
                {  
                    ...
                    if($final_qty > 0)
                    {
                        array_push($sku_arry, $_product->getSku());
                        $selectHtml .= '<li id='.$_value->getSku().' style='.$style.'>'.'<input type="' . $type . '" class="' . $class . ' ' . $require
               ...
               /*$sku = array();
                foreach ($collection as $data) {
                    if ($data["sku"] != "" && $data["status"] == 1) {
                        $sku[$data["entity_id"]] = $data["sku"];
            ...
            //$skus = json_encode($sku_arry);
            //echo "<pre>";print_r($sku_arry);
            
            //echo json_encode($sku_arry);
            ?>
<script type="text/javascript">
                jQuery(document).ready(function () {
                    var jsondata = '<?php echo json_encode($sku_arry); ?>';
                    jsondata = jQuery.parseJSON(jsondata);
                    //console.log(jsondata);
                    for (i = 0; i < jsondata.length; i++)
                    {
                        var jsonid = jQuery.trim(jsondata[i]);
                        jQuery(("li#" + jsonid)).addClass("visiblesku");
                    }
                    jQuery(".product-options .options-list:eq(1) li").each(function (index,ele) {

                        if(jQuery(this).hasClass("visiblesku"))
                        {
                            jQuery(this).show();
                        }
                        if(typeof(jQuery(this).attr("id")) === "undefined")
                        {
                            jQuery(this).addClass("default-option");
                        }
                        else if(index!=0)
                        {
                            jQuery(this).hide();
