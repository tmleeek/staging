...
/* END CUSTOM OPTION CODE */
			...
			{
				$datas=substr($importData['meta_keyword'],0,49);

			    $setValue	= $datas;
			}
			
			if($field == 'ean13')
			{   if($product->getData('amazon_ean') == '')
			    {
				   $product -> setData( 'amazon_ean', $importData['ean13']);
				}
			}
		...
		if($new || $this->getBatchParams('reimport_images') == "true") {  //starts CHECK FOR IF REIMPORTING IMAGES TO PRODUCTS IS TRUE
			...
			if(($this->getBatchParams('deleteall_andreimport_images') == "true" && $importData["image"] != "" && $importData["small_image"] != "" && $importData["thumbnail"] != "") || ($this->getBatchParams('deleteall_andreimport_images') == "true" && $importData["gallery"] != '')) {
						...
						if (isset($attributes['media_gallery'])) {
								...
								foreach($galleryData['images'] as $image){
										...
										if ($gallery->getBackend()->getImage($product, $image['file'])) {
										       ...
										       // $imagefile = str_replace("/","\\",$image['file']);
			...
			foreach($group_price_priceData as $singleattributeData) {
				...
				if($FinalGroupedPriceData[0] != "") {
					if($new) {
						...
						#echo "SQL UPDATE: " . $updatePrice;
						$select_qry2 = $read->query("SELECT value_id FROM ".$resource->getTableName('catalog/product_attribute_group_price')." WHERE value = '".$FinalGroupedPriceData[1]."' AND entity_id = '".$product->getId()."'");
						$newrowItemId2 = $select_qry2->fetch();
						$db_product_id = $newrowItemId2['value_id'];
						if($db_product_id == "") {
							$insertPrice='INSERT into '.$resource->getTableName('catalog/product_attribute_group_price').' (entity_id,all_groups,customer_group_id,value,website_id) VALUES ("'.$product->getId().'","0","'.$FinalGroupedPriceData[0].'","'.$FinalGroupedPriceData[1].'","0");';
						...
						#echo "SQL UPDATE2: " . $updatePrice;
						$updatePrice="UPDATE ".$resource->getTableName('catalog/product_attribute_group_price')." SET customer_group_id = '".$FinalGroupedPriceData[0]."', value = '".$FinalGroupedPriceData[1]."' WHERE entity_id = '".$product->getId()."'";
						$adapter->query($updatePrice);
						}
					}
				}
