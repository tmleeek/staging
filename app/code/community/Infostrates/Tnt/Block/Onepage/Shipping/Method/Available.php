...
class Infostrates_Tnt_Block_Onepage_Shipping_Method_Available extends Mage_Checkout_Block_Onepage_Shipping_Method_Available
{
    ...
    public function getCustomerNotes(){
    	if ($notes = Mage::getSingleton('checkout/session')->getQuote()->getShippingAddress()->getCustomerNotes() ){
    		if(!empty($notes)){
    			return unserialize ( $notes );
			}
    	}
    	if ( $notes = Mage::getSingleton('checkout/session')->getCustomerNotes( true ) ){
    		if(!empty($notes) && !empty($notes[0])){
    			return unserialize ( $notes );
    		}
