...
class MDN_Purchase_ProductsController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function MassAssociateToSupplierAction(){
        ...
        if($associationCreatedCount>0){
            Mage::getSingleton('adminhtml/session')->addSuccess($this->__('%s product-supplier link(s) created',$associationCreatedCount));
        }else{
           Mage::getSingleton('adminhtml/session')->addError($this->__('%s product-supplier link(s) created',$associationCreatedCount));
        }
