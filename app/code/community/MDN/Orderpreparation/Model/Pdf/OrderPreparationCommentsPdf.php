...
class MDN_Orderpreparation_Model_Pdf_OrderPreparationCommentsPdf extends MDN_Orderpreparation_Model_Pdf_Pdfhelper {
    ...
    public function getPdf($order = array()) {
        ...
        foreach ($items as $orderItemId => $qty) {
            ...
            if ($product->gettype_id() == 'simple'){
              ...
              if(count($parentIds)>0){
                ...
                if($productParent->getId()>0) {
                  $Parentcollection = $productParent->getAttributeText('gamme_collection_new');
                  $Parentmanufacture = $productParent->getAttributeText('manufacturer');

                  if (!empty($Parentmanufacture) && !empty($Parentcollection))
                        $Parent_name = $productParent->getName() . PHP_EOL . $Parentcollection . ' - ' . $Parentmanufacture;
                  elseif (!empty($Parentcollection) && empty($Parentmanufacture))
                        $Parent_name = $productParent->getName() . PHP_EOL . $Parentcollection;
                  elseif (!empty($Parentmanufacture) && empty($Parentcollection))
                        $Parent_name = $productParent->getName() . PHP_EOL . $Parentmanufacture;
                  else
                        $Parent_name = $productParent->getName();
                  $this->y -= 5;
                  ...
                  $name = $this->WrapTextToWidth($page, $Parent_name, self::X_POS_NAME);
             ...
             //echo $product->getAttributeText('gamme_collection_new');
             //exit;
            ...
            if (!empty($Productmanufacture) && !empty($Productcollection))
                $Parent_name = $product->getName() . PHP_EOL . $Productcollection . ' - ' . $Productmanufacture;
            elseif (!empty($Productcollection) && empty($Productmanufacture))
                $Parent_name = $product->getName() . PHP_EOL . $Productcollection;
            elseif (!empty($Productmanufacture) && empty($Productcollection))
                $Parent_name = $product->getName() . PHP_EOL . $Productmanufacture;
            else
            ...
            if($qtySelected > 1)
            {
              $page->setFont(Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA_BOLD), 12.5);
            }
            ...
            {
                //PICTURE
                $QtypicturePath = Mage::getBaseDir() . DS . 'media' . DS . 'wysiwyg' . DS . 'Alert.png';
                if (file_exists($QtypicturePath))
                {
                    try
                    {
                        $zendPicture = Zend_Pdf_Image::imageWithPath($QtypicturePath);
                        $page->drawImage($zendPicture, self::X_POS_QTY + 22, $this->y-2.5, self::X_POS_QTY + 37, $this->y + 15);
                    }
                    catch (Exception $ex)
                    {
                        //nothing
                ...
                foreach ($items as $ssorderItemId => $ssqty) {
                  ...
                  if ($ssItem->getparent_item_id() == $orderItemId) {
                    ...
                    if($subProductId>0){
                     ...
                     $SubProductcollection = $subProductId->getAttributeText('gamme_collection_new');
                     $SubProductmanufacture = $subProductId->getAttributeText('manufacturer');

                    if (!empty($SubProductmanufacture) && !empty($SubProductcollection))
                        $Subproduct_name = $subProductId->getName() . PHP_EOL . $SubProductcollection . ' - ' . $SubProductmanufacture;
                    elseif (!empty($SubProductcollection) && empty($SubProductmanufacture))
                        $Subproduct_name = $subProductId->getName() . PHP_EOL . $SubProductcollection;
                    elseif (!empty($SubProductmanufacture) && empty($SubProductcollection))
                        $Subproduct_name = $subProductId->getName() . PHP_EOL . $SubProductmanufacture;
                    else
                        $Subproduct_name = $subProductId->getName();
                     ...
                     $name = $this->WrapTextToWidth($page, $ssqty.'x '.$Subproduct_name, self::X_POS_NAME);
