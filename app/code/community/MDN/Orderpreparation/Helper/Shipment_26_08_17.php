...
class MDN_Orderpreparation_Helper_Shipment extends Mage_Core_Helper_Abstract
{
        ...
        try
        {
            ...
            if ($_shippingMethod[0] == 'tnt')
            {
                ...
                if($parcels['pdfLabels'] && !file_exists($path.$filename))
                {
                    ...
                    {
                        Throw new Exception( $this->__("Impossible de cr褲 le BT. V豩fiez que le repertoire /media/pdf_bt/ �es droits en 袲iture.") );
                    }
                }
            ...
            if($_shippingMethod[0] == 'socolissimo')
            {
                ...
                exit;*/
                //$shipping_method_code = str_replace("socolissimo_", "", $_order->getShippingMethod());

                //$shippingapimethodid = Mage::getModel('tatvashipping/apimethod')->getApidetails($shipping_method_code);

                //$api_code = $_order->getShippingMethod();
                //$api_shipping_name = $_order->getShippingDescription();

                $shipping_method_code = str_replace("socolissimo_", "", $_order->getShippingMethod());

                $shippingapimethodid = Mage::getModel('tatvashipping/apimethod')->getApidetails($shipping_method_code);

                $api_code = $shippingapimethodid['api_shipping_code'];
                $api_shipping_name = $shippingapimethodid['shipping_method_name'];
                $api_shipping_title = $_order->getShippingDescription();

                $admin = Mage::getSingleton('admin/session')->getUser();
                $shipmentStoreId = $_order->getStoreId();
                ...
                $api_detials = Mage::getStoreConfig('carriers/socolissimo/api_url');
                $api_account = Mage::getStoreConfig('carriers/socolissimo/api_account');
                $api_key = Mage::getStoreConfig('carriers/socolissimo/api_key');
                //$client = new SoapClient('http://api.envoidunet.com/?wsdl');

                $client = new SoapClient($api_detials);
                ...
                $recipientCountryCode = $address->getCountryId();

                $special_region_code = array("CE", "GP", "MQ", "GF","RE", "PM", "YT", "TF","WF", "PF", "NC", "MC", "MF", "BL");

                if (in_array($recipientRegionCode, $special_region_code) && $recipientCountryCode == "FR")
                {
                    $recipientCountryCode = $recipientRegionCode;
                }
                //echo "Country_code =".$recipientCountryCode."<br />";
                //echo "Weight =".$_POST['weight'];
                //exit;
                $mobile_phone = $address->getMobilephone();

                if(empty($mobile_phone))
                    $mobile_phone =  $address->getTelephone();
                ...
                $params->reference = $_order->getIncrementId();
                $params->value = 100;
                $params->carrier = $api_code;
                $params->format = 'A6';

                $params->from = new stdClass();
                $params->from->company = $originStreet2;
                $params->from->firstname = $admin->getFirstname();
                $params->from->lastname = $admin->getLastname();
                $params->from->address1 = trim($originStreet1);
                $params->from->postcode = Mage::getStoreConfig(self::XML_PATH_STORE_ZIP, $shipmentStoreId);
                $params->from->city = Mage::getStoreConfig(self::XML_PATH_STORE_CITY, $shipmentStoreId);
                $params->from->country = Mage::getStoreConfig(self::XML_PATH_STORE_COUNTRY_ID, $shipmentStoreId);
                $params->from->email = $admin->getEmail();
                $params->from->phone = $storeInfo->getPhone();

                $params->to = new stdClass();
                $params->to->company = $address->getCompany();
                $params->to->firstname = $address->getFirstname();
                $params->to->lastname = $address->getLastname();
                $params->to->address1 = trim($address->getStreet1());
                $params->to->address2 = trim($address->getStreet2());
                $params->to->address3 = trim($address->getStreet3());

                $params->to->postcode = $address->getPostcode();
                $params->to->city = $address->getCity();
                $params->to->state = $recipientRegionCode;
                $params->to->country = $recipientCountryCode;
                $params->to->email = $address->getEmail();
                $params->to->phone = $mobile_phone;
               ...
               if(!empty($itemsCollection))
               {
                    ...
                    {
                        $product->qty = $item->getQtyShipped();
                        $product->price = $item->getPrice();
                        $product->name = $item->getName();
                        $product->weight = $item->getWeight()*$item->getQtyShipped();
                    }
                ...
                try
                {
                    ...
                    if ($result['error']->error_id > 0)
                    {
                        ...
                        echo $message."\n";
                        echo "<br /><br /><br />"."data as below:-"."<br /><br />";
                        "<pre>";
                        print_r($params);
                        "</pre>";
                        exit;
                    ...
                    //echo "<pre>";
                    //var_dump($response_data->shipments[0]);
                    //echo "</pre>";
                    $pdf = $response_data->shipments[0]->pdf;
                ...
                $pdf_data = file_get_contents ($pdf);

                //$outputPdf = $this->_combineLabelsPdf($pdf_data);

                $binaryPdf = null;
                if( preg_match_all('#%PDF-(.*?)%%EOF#s', $pdf_data, $matches, PREG_SET_ORDER, 0)) {
                    if( isset($matches[0][0]) ) {
                        $binaryPdf = $matches[0][0];
                    }
                }

                $shipment->setShippingLabel($binaryPdf);

                $tracking_number = $response_data->shipments[0]->trackingnumber;

                $track = Mage::getModel('sales/order_shipment_track')
                        ->setNumber($tracking_number)
                        ->setCarrier("Envoidunet")
                        ->setCarrierCode($_shippingMethod[0])
                        ->setTitle($api_shipping_title)
                        ->setPopup(1);
                $shipment->addTrack($track);
     ...
     * @return Zend_Pdf
     */
    protected function _combineLabelsPdf($content)
    {
