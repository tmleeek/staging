    ...
    public function saveBarcodesFromString($productId, $string) {
        ...
        if ($this->useStandardErpBarcodeManagement()) {
            ...
            if (Mage::getStoreConfig('advancedstock/general/avoid_magento_auto_reindex')) {
              Mage::getSingleton('catalog/Resource_Product_Action')->updateAttributes(array($productId), array($this->getBarcodeAttribute() => $string), 0);
            }else{
              Mage::getSingleton('catalog/product_action')->updateAttributes(array($productId), array($this->getBarcodeAttribute() => $string), 0);
            }
        }
    }
