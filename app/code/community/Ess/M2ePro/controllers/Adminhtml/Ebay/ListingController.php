...
class Ess_M2ePro_Adminhtml_Ebay_ListingController extends Ess_M2ePro_Controller_Adminhtml_Ebay_MainController
{
    ...
    {
        /** @var Ess_M2ePro_Block_Adminhtml_Ebay_Listing_View_Settings_Grid_Motor_EditMode $block */
        $block = $this->getLayout()->createBlock('M2ePro/adminhtml_ebay_listing_view_settings_grid_motor_editMode');
    ...
    private function solveEbayItemDuplicateList(Ess_M2ePro_Model_Listing_Product $listingProduct, array &$messages)
    {
    ...
    protected function processConnector($action, array $params = array())
    {
        ...
        $params['is_realtime'] = (bool)Mage::helper('M2ePro')->jsonDecode($this->getRequest()->getParam('is_realtime'));
        ...
        if ($result == Ess_M2ePro_Helper_Data::STATUS_ERROR) {
            return Mage::helper('M2ePro')->jsonEncode(array('result'=>'error','action_id'=>$actionId));
        }

        if ($result == Ess_M2ePro_Helper_Data::STATUS_WARNING) {
            return Mage::helper('M2ePro')->jsonEncode(array('result'=>'warning','action_id'=>$actionId));
        }

        if ($result == Ess_M2ePro_Helper_Data::STATUS_SUCCESS) {
            return Mage::helper('M2ePro')->jsonEncode(array('result'=>'success','action_id'=>$actionId));
        }
