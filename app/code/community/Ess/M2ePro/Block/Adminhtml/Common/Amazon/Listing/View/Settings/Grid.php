    ...
    extends Ess_M2ePro_Block_Adminhtml_Magento_Product_Grid_Abstract
{
    ...
    protected function _prepareCollection()
    {
        ...
        $collection->joinTable(
            array('ts' => 'M2ePro/Amazon_Template_ShippingTemplate'),
    ...
    protected function _prepareColumns()
    {
        ...
        $title = Mage::helper('M2ePro')->__('Shipping Override Policy');

        if ($account->getChildObject()->isShippingModeTemplate()) {
            $indexField = 'template_shipping_template_title';
            $title = Mage::helper('M2ePro')->__('Shipping Template Policy');
        }
    ...
    protected function getGroupOrder()
    {
        ...
        $groups = array(
            ...
            'edit_template_shipping'    => Mage::helper('M2ePro')->__('Shipping Override Policy')
        );

        if ($account->getChildObject()->isShippingModeTemplate()) {
            $groups['edit_template_shipping'] = Mage::helper('M2ePro')->__('Shipping Template Policy');
        }
    ...
    protected function getColumnActionsItems()
    {
        ...
        if ($account->getChildObject()->isShippingModeOverride()) {
    ...
    protected function _prepareMassaction()
    {
        ...
        $groups = array(
            ...
            'shipping_policy'                   => Mage::helper('M2ePro')->__('Shipping Override Policy'),
            'edit_template_product_tax_code'    => Mage::helper('M2ePro')->__('Product Tax Code Policy'),
            'other'                             => Mage::helper('M2ePro')->__('Other'),
        );

        if ($account->getChildObject()->isShippingModeTemplate()) {
            $groups['shipping_policy'] = Mage::helper('M2ePro')->__('Shipping Template Policy');
        }
...
HTML;
        ...
        if ($account->getChildObject()->isShippingModeTemplate() &&
            ...
            $url = $this->getUrl('*/adminhtml_common_amazon_template_shippingTemplate/edit', array(
                'id' => $row->getData('template_shipping_template_id')
            ));

            $templateTitle = Mage::helper('M2ePro')->escapeHtml($row->getData('template_shipping_template_title'));
...
HTML;
    ...
    public function callbackColumnTemplateProductTaxCode($value, $row, $column, $isExport)
    {
        ...
        if ($row->getData('template_product_tax_code_id')) {
            ...
            $url = $this->getUrl('*/adminhtml_common_amazon_template_productTaxCode/edit', array(
