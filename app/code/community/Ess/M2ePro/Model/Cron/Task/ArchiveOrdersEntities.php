...
final class Ess_M2ePro_Model_Cron_Task_ArchiveOrdersEntities extends Ess_M2ePro_Model_Cron_Task_Abstract
{
    ...
    private function getAffectedOrdersGroupedByComponent()
    {
        ...
        $firstAffectedId = $connRead->select()
            ->from(
                Mage::getSingleton('core/resource')->getTableName('M2ePro/Order'),
                array('id')
            )
            ->order('id DESC')
    ...
    private function processComponentEntities($componentName, array $componentOrdersIds)
    {
        ...
        $mainOrderTable = $coreResource->getTableName('M2ePro/Order');
        $componentOrderTable = $coreResource->getTableName('M2ePro/' .ucfirst($componentName). '_Order');
        ...
        $mainOrderItemTable = $coreResource->getTableName('M2ePro/Order_Item');
        $componentOrderItemTable = $coreResource->getTableName('M2ePro/' .ucfirst($componentName). '_Order_Item');
        ...
        $connWrite->insertMultiple($coreResource->getTableName('M2ePro/ArchivedEntity'), $insertsData);
