    ...
    extends Ess_M2ePro_Model_Ebay_Template_Builder_Abstract
{
    ...
    protected function prepareGeneralData(array &$data)
    {
        ...
        if (isset($data['local_shipping_discount_profile_id'])) {
            $prepared['local_shipping_discount_profile_id'] = Mage::helper('M2ePro')->jsonEncode(
                array_diff($data['local_shipping_discount_profile_id'], array(''))
            );
        } else {
            $prepared['local_shipping_discount_profile_id'] = Mage::helper('M2ePro')->jsonEncode(array());
        }

        if (isset($data['international_shipping_discount_profile_id'])) {
            $prepared['international_shipping_discount_profile_id'] = Mage::helper('M2ePro')->jsonEncode(
                array_diff($data['international_shipping_discount_profile_id'], array(''))
            );
        } else {
            $prepared['international_shipping_discount_profile_id'] = Mage::helper('M2ePro')->jsonEncode(array());
        }
