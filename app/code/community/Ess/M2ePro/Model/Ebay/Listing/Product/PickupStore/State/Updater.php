...
class Ess_M2ePro_Model_Ebay_Listing_Product_PickupStore_State_Updater
{
    ...
    public function process()
    {
        ...
        if (!$this->isDeleted()) {
            ...
            $connWrite->update(
                $resource->getTableName('m2epro_ebay_listing_product_pickup_store'),
    ...
    private function applyCalculatedValues(array $calculatedValues)
    {
        ...
        if (!empty($calculatedValues['create'])) {
            $connWrite->insertMultiple(
                $resource->getTableName('m2epro_ebay_account_pickup_store_state'), $calculatedValues['create']
            );
        ...
        foreach ($calculatedValues['update'] as $qty => $filters) {
            ...
            $connWrite->update(
                $resource->getTableName('m2epro_ebay_account_pickup_store_state'),
