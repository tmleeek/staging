    ...
    extends Ess_M2ePro_Model_Buy_Synchronization_OtherListings_Abstract
{
    ...
    protected function intervalIsLocked()
    {
        ...
        $operationHistory = $this->getActualOperationHistory()->getParentObject('synchronization_buy');
        ...
        $updateListingsProductsLastTime = $this->getConfigValue(
            '/buy/listings_products/update/', 'last_time'
        );
        ...
        foreach ($accounts as $account) {
            ...
            try {
                ...
                $message = Mage::helper('M2ePro')->__(
                    'The "3rd Party Listings" Action for Rakuten.com Account: "%account%" was completed with error.',
                    $account->getTitle()
                );
    ...
    private function processAccountInventory(Ess_M2ePro_Model_Account $account)
    {
        ...
        if (!$this->isLockedAccount($account)) {
            ...
            $connectorObj = $dispatcherObject->getCustomConnector(
                'Buy_Synchronization_OtherListings_Update_Requester',
                array(), $account
            );
    ...
    private function isLockedAccount(Ess_M2ePro_Model_Account $account)
    {
        ...
        $lockItem->setNick(
            Ess_M2ePro_Model_Buy_Synchronization_OtherListings_Update_ProcessingRunner::LOCK_ITEM_PREFIX
            .'_'.$account->getId()
        );
