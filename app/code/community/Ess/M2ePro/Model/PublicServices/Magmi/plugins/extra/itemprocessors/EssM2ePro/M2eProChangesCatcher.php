...
class M2eProChangesCatcher extends Magmi_ItemProcessor
{
    ...
    const CHANGE_INITIATOR_MAGMI_PLUGIN = 5;
    ...
    {
        $this->changes[(int)$params['product_id']] = array(
    ...
    private function filterOnlyAffectedChanges()
    {
        ...
        foreach ($this->changes as $productId => $change) {
            ...
            if (!in_array($productId, $productsInListings)) {
                $this->statistics['not_presented']++;
                unset($this->changes[$productId]);
            }
        }
    }
    ...
    private function insertChanges()
    {
        ...
        foreach (array_chunk($this->changes, 500, true) as $productChangesPart) {
            ...
            while ($row = $stmt->fetch()) {
                $existedChanges[] = (int)$row['product_id'];
            }
        }
        ...
        $this->log('There are '. count($existedChanges) .' changes already stored in the m2epro_changes_table.');

        foreach ($this->changes as $productId => $change) {

            if (in_array($productId, $existedChanges)) {
                $this->statistics['existed']++;
                continue;
            }

            $this->statistics['inserted']++;
            $this->insert($insertSql, array($productId,
                                            self::CHANGE_UPDATE_ACTION,
                                            self::CHANGE_UPDATE_ATTRIBUTE_CODE,
                                            self::CHANGE_INITIATOR_MAGMI_PLUGIN,
                                            date('Y-m-d H:i:s'),
                                            date('Y-m-d H:i:s')));
        }
