...
class DropboxFile
{
     ...
     * File Stream
     *
     * @var \MageBackup\GuzzleHttp\Psr7\Stream
     */
    protected $stream;
    ...
    public static function createByStream($fileName, $resource, $mode = self::MODE_READ)
    {
        ...
        $stream = \MageBackup\GuzzleHttp\Psr7\stream_for($resource);
        if (!$stream) {
     ...
     *
     * @return \MageBackup\GuzzleHttp\Psr7\Stream
     */
    ...
    public function open()
    {
        ...
        // Create a stream
        $this->stream = \MageBackup\GuzzleHttp\Psr7\stream_for(fopen($this->path, $this->mode));
    ...
    {
        return \MageBackup\GuzzleHttp\Psr7\mimetype_from_filename($this->path) ?: 'text/plain';
    }
}
