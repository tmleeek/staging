...
namespace MageBackup\Aws\Common\Client;

use MageBackup\Aws\Common\Credentials\Credentials;
use MageBackup\Aws\Common\Credentials\CredentialsInterface;
use MageBackup\Aws\Common\Credentials\NullCredentials;
use MageBackup\Aws\Common\Enum\ClientOptions as Options;
use MageBackup\Aws\Common\Exception\ExceptionListener;
use MageBackup\Aws\Common\Exception\InvalidArgumentException;
use MageBackup\Aws\Common\Exception\NamespaceExceptionFactory;
use MageBackup\Aws\Common\Exception\Parser\DefaultXmlExceptionParser;
use MageBackup\Aws\Common\Exception\Parser\ExceptionParserInterface;
use MageBackup\Aws\Common\Iterator\AwsResourceIteratorFactory;
use MageBackup\Aws\Common\RulesEndpointProvider;
use MageBackup\Aws\Common\Signature\EndpointSignatureInterface;
use MageBackup\Aws\Common\Signature\SignatureInterface;
use MageBackup\Aws\Common\Signature\SignatureV2;
use MageBackup\Aws\Common\Signature\SignatureV3Https;
...
use MageBackup\Guzzle\Service\Description\ServiceDescription;
use MageBackup\Guzzle\Service\Resource\ResourceIteratorClassFactory;
use MageBackup\Guzzle\Log\LogAdapterInterface;
use MageBackup\Guzzle\Log\ClosureLogAdapter;
use MageBackup\Guzzle\Plugin\Backoff\BackoffLogger;
...
class ClientBuilder
{
    ...
    public function __construct($namespace = null)
    {
        ...
        $this->clientClass = 'MageBackup\Aws\Common\Client\DefaultClient';
            ...
            if ($logger === 'debug') {
                ...
                throw new InvalidArgumentException(
                    Options::BACKOFF_LOGGER . ' must be set to `debug` or an instance of '
                        . 'MageBackup\Guzzle\\Common\\Log\\LogAdapterInterface'
                );
            }
     ...
     * version identifier string or an instance of MageBackup\Aws\Common\Signature\SignatureInterface.
     *
    ...
    protected function getSignature(ServiceDescription $description, Collection $config)
    {
        ...
        if (is_string($signature)) {
            ...
            throw new InvalidArgumentException('The provided signature is not '
                ...
                . 'MageBackup\Aws\\Common\\Signature\\SignatureInterface');
        }
