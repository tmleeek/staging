...
namespace MageBackup\Aws\Common\Credentials;

use MageBackup\Aws\Common\Enum\ClientOptions as Options;
use MageBackup\Aws\Common\Exception\InvalidArgumentException;
use MageBackup\Aws\Common\Exception\RequiredExtensionNotLoadedException;
use MageBackup\Aws\Common\Exception\RuntimeException;
use MageBackup\Guzzle\Common\FromConfigInterface;
use MageBackup\Guzzle\Cache\CacheAdapterInterface;
use MageBackup\Guzzle\Cache\DoctrineCacheAdapter;
        ...
        if ($cache === 'true' || $cache === true) {
            ...
            if (!extension_loaded('apc')) {
                throw new RequiredExtensionNotLoadedException('PHP has not been compiled with APC. Unable to cache '
                    . 'the credentials.');
            } elseif (!class_exists('MageBackup\Doctrine\Common\Cache\ApcCache')) {
                throw new RuntimeException(
                    ...
                    . 'MageBackup\Guzzle\Cache\CacheAdapterInterface object'
                );
            }
            // @codeCoverageIgnoreEnd
            $cache = new DoctrineCacheAdapter(new \MageBackup\Doctrine\Common\Cache\ApcCache());
