...
use MageBackup\GuzzleHttp\Promise\EachPromise;
...
class CommandPool implements PromisorInterface
{
            ...
            foreach ($commands as $key => $command) {
                if (!($command instanceof CommandInterface)) {
                    throw new \InvalidArgumentException('Each value yielded by '
                        . 'the iterator must be an MageBackup\Aws\CommandInterface.');
                }
                if ($before) {
                    $before($command, $key);
                }
     ...
     *
     * @return array
     * @see \MageBackup\Aws\CommandPool::__construct for available configuration options.
     */
