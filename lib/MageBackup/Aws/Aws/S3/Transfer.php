...
use MageBackup\GuzzleHttp\Promise\PromisorInterface;
use Iterator;
...
class Transfer implements PromisorInterface
{
     ...
     *   string $dest, MageBackup\Aws\CommandInterface $command. The provided command will
    ...
    private function createDownloadPromise()
    {
        ...
        return (new MageBackup\Aws\CommandPool($this->client, $commands, [
    ...
    private function createUploadPromise()
    {
        ...
        $files = \MageBackup\Aws\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return MageBackup\Aws\filter(
                MageBackup\Aws\recursive_dir_iterator($this->sourceMetadata['path']),
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = $this->client
                ...
                ->search('Contents[].Key');
            $files = MageBackup\Aws\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/$key";
            });
            return MageBackup\Aws\filter($files, function ($key) {
