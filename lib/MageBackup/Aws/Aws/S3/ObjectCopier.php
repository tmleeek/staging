...
use MageBackup\Aws\Exception\MultipartUploadException;
...
class ObjectCopier implements PromisorInterface
{
     ...
     *
     * @return \MageBackup\GuzzleHttp\Promise\Promise
     */
    public function promise()
    {
        return \MageBackup\GuzzleHttp\Promise\coroutine(function () {
    ...
    private function validateLocation(array $location)
    {
        if (empty($location['Bucket']) || empty($location['Key'])) {
            throw new \InvalidArgumentException('Locations provided to an'
                . ' MageBackup\Aws\S3\ObjectCopier must have a non-empty Bucket and Key');
        }
    }
