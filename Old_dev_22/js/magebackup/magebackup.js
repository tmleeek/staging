	...
	MageBackup.$.ajax({
		...
		success:	function(data) {
			if (data.error) {
				...
				MageBackup.$('#dropbox_token').val(data.token['oauth_token']);
				MageBackup.$('#dropbox_tokensecret').val(data.token['oauth_token_secret']);
				MageBackup.$('#dropbox_userid').val(data.token['uid']);
