...
var RedemptionSlider = Class.create({
    ...
    */
    initialize: function(sliderHandleId, sliderRailId, sliderCaptionId, sliderValueboxId) {
        ...
        /* Some restyling for Magento EE and Gift Cards */
        var rewardsCartpointsSpenderBox = $$('.rewards-cartpoints-spender-box');
        var giftCardAddInput = $('giftcardaccount_add_input');
        if (rewardsCartpointsSpenderBox.length === 1 && giftCardAddInput) {
            var giftCardBox = giftCardAddInput.up('.entry-edit');
            if (giftCardBox) {
                giftCardBox.removeClassName('box-right');
                giftCardBox.addClassName('box-left');
                giftCardBox.setStyle({clear: 'left'});            	
            }
        }
    },
    
    /**
     * register a listener for the onChange event
     */
    ...
    changeListener: function(val) {
        this.update(val);
        if (typeof this.onChangeCallback != 'undefined') {
    ...
    slideListener: function(val) {
        this.update(val);
        if (typeof this.onSlideCallback != 'undefined') {
    ...
    regenerateSlider: function(min, max, step, initial_value) {
        ...
        this.slider.useSmoothSliding = true;
    },
