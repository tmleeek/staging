...
class MDN_ErpApi_Model_Api_V2 extends Mage_Api_Model_Resource_Abstract {
    ...
    */
    ...
    public function createStockTake($stockTakeName, $warehouseId) {
        ...
        try{
            ...
            $inventory->setei_warehouse_id($warehouseId)
                ...
                //->setei_status($inventory::kStatusOpened);
            $inventory->save();
    ...
    public function getStockPicture($stockTakeId)
    {
        ...
        {
            $p = $this->productInfosById($item['entity_id']);

            //$product = Mage::getModel('Catalog/Product')->load($item['entity_id']);
            //$p['id'] = $item['entity_id'];
            //$p['sku'] = $item['sku'];
            //$p['ean'] = $product->getBarcode();
            //$p['name'] = $item['name'];
    ...
    /**
     * get the scanned products
     *
     * @param $stockTakeId
     * @return array
     */
    public function getScannedProducts($stockTakeId)
    {
        $inventory = Mage::getModel('AdvancedStock/Inventory')->load($stockTakeId);
        $scannedProducts = $inventory->getScannedProducts();
        ...
        foreach($scannedProducts as $scannedProduct)
        {
            ...
            $product = Mage::getModel('Catalog/Product')->load($scannedProduct['entity_id']);
            $p['id'] = (int)$scannedProduct['entity_id'];
            $p['sku'] = $scannedProduct['sku'];
            //$p['ean'] = $product->getBarcode();
            $p['ean'] = $this->getBarcodesFromProductId($product->getId());
            $p['name'] = $scannedProduct['name'];
            $p['scanned'] = (int)$scannedProduct['scanned_qty'];
            $p['expected'] = (int)$scannedProduct['expected_qty'];
            $p['location'] = $scannedProduct['shelf_location'];
