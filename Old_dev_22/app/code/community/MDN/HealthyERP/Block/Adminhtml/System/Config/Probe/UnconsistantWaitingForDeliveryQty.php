...
class MDN_HealthyERP_Block_Adminhtml_System_Config_Probe_UnconsistantWaitingForDeliveryQty extends MDN_HealthyERP_Block_Adminhtml_System_Config_Probe_Abstract
{
    ...
    private static function getErrorsList(){
        ...
        $sql = "select
                ...
                from
                    ".$tableprefix."purchase_order
                    inner join ".$tableprefix."purchase_order_product on (pop_order_num = po_num)
                where
        ...
        $sql = "select
                ...
                from
                    ".$tableprefix."catalog_product_entity tbl_product
                    inner join ".$tableprefix."catalog_product_entity_int tbl_wfd on (tbl_product.entity_id = tbl_wfd.entity_id and store_id = 0)
                    inner join ".$tableprefix."eav_attribute tbl_attribute on (tbl_attribute.attribute_id = tbl_wfd.attribute_id)
                where
        ...
        //tranform arrays to get product id as key
        $t = array();
    ...
    protected function getCurrentSituation()
    {
      ...
      if (Mage::getStoreConfig('healthyerp/options/display_basic_message')){
        $situation = $this->__('Products with a Waiting for delivery Qty issue').' : '.$this->_productCountToFix.'<br/>';
