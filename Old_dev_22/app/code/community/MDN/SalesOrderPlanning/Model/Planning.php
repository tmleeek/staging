	...
	******************************************************************************************************************************/
	...
	public function setShippingInformation($order, $quoteMode = false)
	{
		...
		if (Mage::getStoreConfig('planning/shipping/avoid_holy_days') == 1)
		{
            ...
            /* for type added by mauli (holiday for socolissimo) */
            if (!$quoteMode)
            ...
            $type=0;
            if(($shipping_code[0]=='socolissimo') || ($carrier=='colissimo_colissimo') || ($carrier=='colissimoappointment_colissimoappointment') || ($carrier=='colissimolocalstore_colissimolocalstore') || ($carrier=='colissimopostoffice_colissimopostoffice') || ($carrier=='colissimocityssimo_colissimocityssimo') || ($carrier=='tnt_JD') || ($carrier=='tnt_J') ||($carrier=='tnt_JZ'))
             {
                $type=1;
             }
            /* end --mauli */
			$shippingDateTimeStamp = $holidayHelper->addDaysWithoutHolyDays(strtotime($fullstockDate), $orderPreparationDuration,$type);
	...
	public function setDeliveryInformation($order, $quoteMode = false)
	{
        ...
        $shipping_code = explode('_', $carrier);
        /* add code by mauli */
         $type=0;
            if(($shipping_code[0]=='socolissimo') || ($carrier=='colissimo_colissimo') || ($carrier=='colissimoappointment_colissimoappointment') || ($carrier=='colissimolocalstore_colissimolocalstore') || ($carrier=='colissimopostoffice_colissimopostoffice') || ($carrier=='colissimocityssimo_colissimocityssimo') || ($carrier=='tnt_JD') || ($carrier=='tnt_J') ||($carrier=='tnt_JZ'))
             {
        ...
        //echo "shippingDelay =".$shippingDelay;

		//avoid holy day (if set)
		if (Mage::getStoreConfig('planning/delivery/avoid_holy_days') == 1)
		{
			$holidayHelper = mage::helper('SalesOrderPlanning/Holidays');
			$deliveryDateTimeStamp = $holidayHelper->addDaysWithoutHolyDays($shippingDateTimestamp, $shippingDelay,$type);
