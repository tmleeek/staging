...
class MDN_Mpm_Model_Export_Catalog extends Mage_Core_Model_Abstract
{
    ...
    public function getProductsCollection($startProductId = 0)
    {
        ...
        if(!empty(Mage::getStoreConfig('advancedstock/barcode/barcode_attribute'))) {
            $this->_collection->addAttributeToSelect(Mage::getStoreConfig('advancedstock/barcode/barcode_attribute'));
        }
        ...
        $joinType = 'left';
        if (Mage::getStoreConfig('mpm/catalog_export/export_only_with_stock')) {
            $joinType = 'inner';
        }
        ...
        if (Mage::getStoreConfig('mpm/catalog_export/product_types')) {
            $this->_collection->addFieldToFilter('type_id', array('in' => explode(',', Mage::getStoreConfig('mpm/catalog_export/product_types'))));
        }

        if (Mage::getStoreConfig('mpm/catalog_export/product_visibilities')) {
            $this->_collection->addAttributeToFilter('visibility', array('in' => explode(',', Mage::getStoreConfig('mpm/catalog_export/product_visibilities'))));
        }

        if (Mage::getStoreConfig('mpm/catalog_export/allowed_manufacturers')) {
            $this->_collection->addAttributeToFilter('manufacturer', array('in' => explode(',', Mage::getStoreConfig('mpm/catalog_export/allowed_manufacturers'))));
        }

        if (Mage::getStoreConfig('mpm/catalog_export/created_at_after')) {
            $tmp = explode('/', Mage::getStoreConfig('mpm/catalog_export/created_at_after'));
            $this->_collection->addFieldToFilter('created_at', array('from' => $tmp[2] . '-' . $tmp[1] . '-' . $tmp[0]));
        }

        if (Mage::getStoreConfig('mpm/catalog_export/created_at_before')) {
            $tmp = explode('/', Mage::getStoreConfig('mpm/catalog_export/created_at_before'));
    ...
    {
        if (!Mage::getStoreConfig('advancedstock/barcode/barcode_attribute')) {
