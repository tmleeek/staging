...
class MDN_ProductReturn_AdminController extends Mage_Adminhtml_Controller_Action
{
     ...
     *
     */
    public function NotifyreturnlabelAction()
    {
        ...
        try {
            ...
            //j envoi un mail au client
            $rma->Notifyreturnlabel();

            //confirme
            Mage::getSingleton('adminhtml/session')->addSuccess($this->__('Return label successfully notified.'));
        } catch (Exception $ex) {
            Mage::getSingleton('adminhtml/session')->addError($this->__('Unable to send return label') . ' : ' . $ex->getMessage());
        }
    ...
    private function initDataCreditMemo($rma)
    {
        ...
        $this->_CreditMemoCreationData['refund_shipping_amount'] = $data['refund_shipping_amount'];
