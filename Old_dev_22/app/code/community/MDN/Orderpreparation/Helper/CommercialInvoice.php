...
class MDN_Orderpreparation_Helper_CommercialInvoice extends Mage_Core_Helper_Abstract
{
    ...
    public function _HtmltoPdf($shipmentIncrmentId,$html = null)
    {
        $mpdf = new Mpdf();
        $mpdf->WriteHTML($html);
        $filename = "commercialinvoice_" . $shipmentIncrmentId . ".pdf";
    ...
    {        
        ...
        $html .= '<h3 align="center">COMMERCIAL INVOICE #' . $shipment->getIncrementId() . '</h3>';
        ...
        foreach ($shipment->getAllItems() as $item) {
            ...
            $html .= '<tr>';            
            $productMenu = ($Product->getCodeDouanier()) ? "TARIC: ".$Product->getCodeDouanier() : $Product->getAttributeText('manufacturer');
            $html .= '<td style="border:1px solid #333;">' . $Product->getName()."<br/>". $productMenu ."<br/>". $Product->getAttributeText('gamme_collection_new') .'</td>';
            $html .= '<td style="border:1px solid #333;">' . $manufacturer . '</td>';
            $html .= '<td style="border:1px solid #333;">N</td>';
            $html .= '<td style="border:1px solid #333;">' . (int)$item->getQty() . '</td>';
            $html .= '<td style="border:1px solid #333;">' . number_format($item->getPrice(), 2) . '</td>';
            $html .= '<td style="border:1px solid #333;">' . number_format($Total, 2) . '</td>';
            $html .= '</tr>';
        }
        $html .= '</tbody>';
        $html .= '</table>';
        ...
        $html .= '</tr>';
        $html .= '<tr>';
        $html .= '<td>exon&#233;ration de TVA, article 262-I de CGI</td>';
        $html .= '</tr>';
        $html .= '</table>';
        $html .= '</td> ';
        ...
        $html .= '<tr><td height="20px"><img src="'.Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'commercialinvoice'.DS.'stamp_sign.png'.'" height=120 width=250 /></td> <td height="20px"></td></tr>';
        $html .= '<tr>';
        ...
        $invoiceDate = date('M j, Y',strtotime($shipment->getCreatedAt()));
        ...
        //echo $html;
        //exit;
        $this->_HtmltoPdf($shipment->getIncrementId(),$html);
    }
}
