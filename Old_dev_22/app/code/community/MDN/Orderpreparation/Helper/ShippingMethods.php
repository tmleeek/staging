...
class MDN_Orderpreparation_Helper_ShippingMethods extends Mage_Core_Helper_Abstract {
    ...
    public function changeForOrder($orderId, $shippingMethod)
    {
        ...
        if($typeSocolissimoArray[0]=="socolissimo")
        {
            $shipping_code = $this->getSocoProductCode($typeSocolissimo, $typeSocolissimoDomicile);
            ...
            $order->setshipping_description($shippingDescription)
                ...
                ->setsoco_product_code($shipping_code)
                ->save();
        }
        else
        {
    ...
    public function getSocoProductCode ($type, $domicile, $countryId)
    {
        if ($type == 'poste')
        {
            return 'BPR';
