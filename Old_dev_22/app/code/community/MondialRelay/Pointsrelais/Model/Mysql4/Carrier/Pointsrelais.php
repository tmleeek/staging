<?php
class MondialRelay_Pointsrelais_Model_Mysql4_Carrier_Pointsrelais extends Mage_Core_Model_Mysql4_Abstract
{
	protected function _construct()
	{
		$this->_init('shipping/pointsrelais', 'pk');
	}

    public function getmondialrate($country_id,$weight)
	{

    $read = $this->_getReadAdapter();

		$table = Mage::getSingleton('core/resource')->getTableName('mondialrelay_pointsrelais');

		$select = $read->select()->from($table);

		$select->where("dest_country_id=? ", $country_id);

        $select->where('condition_name=?', 'package_weight');
        $select->where('condition_value>=?', $weight);
	 ...
	 public function getmondialratefromcsv($country_id,$weight)
	{
    ...
    {
        $csvFile = $_FILES["groups"]["tmp_name"]["pointsrelais"]["fields"]["import"]["value"];
        ...
        if (!empty($csvFile))
        {
            ...
            if (isset($_POST['groups']['pointsrelais']['fields']['condition_name']['inherit']))
            {
                $conditionName = (string)Mage::getConfig()->getNode('default/carriers/pointsrelais/condition_name');
            }
            else
            {
                $conditionName = $_POST['groups']['pointsrelais']['fields']['condition_name']['value'];
            }

            $conditionFullName = Mage::getModel('pointsrelais/carrier_pointsrelais')->getCode('condition_name_short', $conditionName);
