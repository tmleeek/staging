...
/**
 * MageBackup mysql4 helper model.
 *
 * @category	MageBackup
 * @package		MageBackup_MageBackup
 * @author		MageBackup Team <admin@magebackup.com>
 */
class MageBackup_MageBackup_Model_Resource_Helper_Mysql4 extends Mage_Core_Model_Resource_Helper_Mysql4 {
	/**
	...
	public function getTableRecords($tableName) {
		$sql	= 'SELECT COUNT(*) FROM ' . $this->getAdapter()->quoteIdentifier($tableName);
	...
	public function getHeader() {
		...
		$versionRow = $this->getAdapter()->fetchRow('SHOW VARIABLES LIKE \'version\'');
		...
		$header = "-- Magento DB backup\n"
				. "-- Powered by MageBackup (http://www.magebackup.com)\n"
				. "-- Dump started on " . Mage::getSingleton('core/date')->gmtDate() . " GMT\n"
				. "--\n"
	...
	public function turnOnSerializableMode() {
		$this->getAdapter()->query("SET SESSION TRANSACTION ISOLATION LEVEL SERIALIZABLE");
	}
	...
	public function turnOnReadCommittedMode() {
		$this->getAdapter()->query("SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED");
	}
}
