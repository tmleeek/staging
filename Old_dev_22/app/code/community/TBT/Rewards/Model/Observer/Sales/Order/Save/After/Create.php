...
class TBT_Rewards_Model_Observer_Sales_Order_Save_After_Create
{
    ...
    protected function _processPendingMsgs($order)
    {
        ...
        if ($order->hasPointsEarning () && !$order->hasPointsSpending ()) {
            ...
            $_SESSION['sucess_msg'] = Mage::helper ( 'rewards' )->__ ( 'You earned %s for the order you just placed.', $earned_points_string );
            ...
            $_SESSION['sucess_msg'] = Mage::helper ( 'rewards' )->__ ( 'You spent %s for the order you just placed.', $redeemed_points_string );
        } elseif ($order->hasPointsEarning () && $order->hasPointsSpending ()) {
            $this->_dispatchSuccess ( Mage::helper ( 'rewards' )->__ ( 'You earned %s and spent %s for the order you just placed.', $earned_points_string, $redeemed_points_string ) );
              $_SESSION['sucess_msg'] =  Mage::helper ( 'rewards' )->__ ( 'You earned %s and spent %s for the order you just placed.', $earned_points_string, $redeemed_points_string );
        } else {
        ...
        if ($order->isInitialTransferStatus(TBT_Rewards_Model_Transfer_Status::STATUS_PENDING_EVENT)) {
            ...
            $_SESSION['sucess_msg_1'] = Mage::helper('rewards')->__('The points you earned are currently pending the completion of your order. You will be able to spend these points after we finish processing your order.');
            ...
            $this->_dispatchSuccess (Mage::helper('rewards')->__('The points you earned are currently on hold until we have processed your order. You will be able to spend these points after an administrator has approved the order.'));
              $_SESSION['sucess_msg_1'] = Mage::helper('rewards')->__('The points you earned are currently on hold until we have processed your order. You will be able to spend these points after an administrator has approved the order.');
        } else {
