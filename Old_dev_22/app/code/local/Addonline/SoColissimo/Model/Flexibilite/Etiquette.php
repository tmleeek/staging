...
class Addonline_SoColissimo_Model_Flexibilite_Etiquette
{
    ...
    public function run(Mage_Shipping_Model_Shipment_Request $request)
    {
        ...
        //$order = $request->getOrderShipment()->getOrder();
        //$request->setRecipientAddresseeParcelRef($order->getEntityId());

        $generateLabelRequest_data = $this->_objectToArray($generateLabelRequest);
        $generateLabelRequest_array = array();
        $generateLabelRequest_array = $generateLabelRequest_data;
        $generateLabelRequest_array['letter']['addressee']= '';
        $generateLabelRequest_array['letter']['addressee']['addresseeParcelRef']= $order->getIncrementId();
        $generateLabelRequest_array['letter']['addressee']['codeBarForReference']= "false";
        $generateLabelRequest_array['letter']['addressee']['serviceInfo']= "service info";
        $generateLabelRequest_array['letter']['addressee']['promotionCode']= '';
        $generateLabelRequest_array['letter']['addressee']['address']= $generateLabelRequest_data['letter']['addressee']['address'];
        //convert object parameter  to XML parameter.
        $xmlRequest = $this->_getXmlRequest($generateLabelRequest_array);
    ...
    private function _prepareServiceOriginal(ServiceCallContext &$service, Mage_Shipping_Model_Shipment_Request $request, Mage_Sales_Model_Order $order)
    {
        $service->setProductCode($order->getSocoProductCode());
        $service->setDepositDate();
        $service->setCommercialName($request->getShipperContactCompanyName());
    ...
    private function _prepareSender(Address &$address, Mage_Shipping_Model_Shipment_Request $request)
    {
        ...
        $address->setLine0($request->getShipperAddressStreet2());
		$address->setLine1($request->getShipperAddressStreet3());
		$address->setLine2($request->getShipperAddressStreet1());
        //$address->setDoorCode1($request->getShipperDoorCode1());
    ...
    private function _prepareAddressee(Address &$address, Mage_Shipping_Model_Shipment_Request $request)
    {
        ...
        $address->setLine0($request->getRecipientAddressStreet2());
		$address->setLine1($request->getRecipientAddressStreet3());
		$address->setLine2($request->getRecipientAddressStreet1());
        ...
        $address->setPhoneNumber($request->getRecipientContactPhoneNumber());
        $address->setMobileNumber($request->getRecipientContactMobileNumber());

        $address->setDoorCode1($request->getRecipientDoorCode1());
        $address->setIntercom($request->getRecipientIntercom());
        $address->setLanguage($request->getRecipientLanguage());

        $address->setEmail('noreply@noreply.com');
		//$address->setPhoneNumber($request->getMobilePhone());

  }
	
	
	private function _prepareAddresseeOriginal(Address &$address, Mage_Shipping_Model_Shipment_Request $request)
    {
        //$order = $request->getOrderShipment()->getOrder();
