...
//echo '<pre>';print_r($datas);exit;
           foreach($datas as $keys => $title)
           {
			...
			//'available_stock',
             ...
             if($datas[$keys]['unique_id'])
             {
              $productAvailabilityStatus = mage::getModel('SalesOrderPlanning/ProductAvailabilityStatus')->load($datas[$keys]['unique_id'], 'pa_product_id');
              $availabilityMessage = '';
                  if ($productAvailabilityStatus) {
                    $availabilityMessage = $productAvailabilityStatus->getMessage();
                 }
                $xml->stock_quantity->addCData($availabilityMessage);
             }
                   ...
                   /*  echo  $percent = Mage::getSingleton('tax/calculation')->getRate($request->setProductClassId($taxclassid));        */
                    ...
                    if($product->getName())
                    {
						...
						if($prod['gamme_collection_new'] != '')
						{
							...
							if ($attr->usesSource()) {
							    $gamme_collection = $attr->getSource()->getOptionText($product->getGammeCollectionNew ());
							}
							if($gamme_collection != '')
							{
                       ...
                       $attr = $productModel->getResource()->getAttribute("manufacturer ");
						if ($attr->usesSource()) {
						    $manufacture_label_1 = $attr->getSource()->getOptionText($product->getManufacturer ());
						}
                    ...
                    //frais de port
                      if($product->getTypeId() != 'virtual')
                      {
								...
								$result = number_format($this->getShippingAmount() * (1 + ($tax / 100)),2);
								//$result = ceil ($result);
                    ...
                    {//echo 'yes';exit;
                      ...
                      $ids = explode('/', $path);
                       ...
                       foreach($ids as $id)
                       {
                          ...
                          if($name!='ROOT' || $name!='Default Category')
                          {
                           $cat_name.=$name.' > ';
                          }
                       }
...
//echo '<pre>';print_r($line);exit;
                    ...
                    // stock qty
                     $available_qty='';
                     $available_qty=$this->getAvailableSupplientTotalQty($prod['sku']);
                     if($available_qty!='')
                    {
                      $line['stock_quantity']=intVal($available_qty);
                    }
                    else
                    {
                     $line['stock_quantity']='';
                    ...
                    $line['delivery_description']='Livraison sous 24 à 48h apres l expedition avec GLS';
	...
	public function getShippingAmount(){
        ...
        $amount = 0;
       /*if(Mage::getStoreConfig('carriers/gls/active')== 1 )
       {
        	$method = Mage::getStoreConfig('sqlidelivery/estimate_delivery/shipping_method');
      		...
      		//pays par défaut
      		$countryId = Mage::getStoreConfig('sqlidelivery/estimate_delivery/country');

      		$rule = Mage::getModel('sqlishipping/rule')
          		->getCollection()
            	->addShippingFilter($shipping->getCode())
          		->addCountryFilter($countryId)
          		->addWeightFilter($this->getWeight())
          		->getFirstItem();
       ...
       if(Mage::getStoreConfig('carriers/pointsrelais/active') == 1)
       {
            $countryId = 'FR';
           $amount = number_format(Mage::getResourceModel('pointsrelais/carrier_pointsrelais')->getmondialrate($countryId,$this->getWeight()),2);
       }
       else
       {
      		//Mode de transport par défaurt
      		$method = Mage::getStoreConfig('sqlidelivery/estimate_delivery/shipping_method');
      		$code = $this->getCodeShippingMethod($method);
          	$carrierModel = Mage::getStoreConfig('carriers/'.$code.'/model');
      		$shipping = Mage::getModel($carrierModel);
      		...
      		$rule = Mage::getModel('sqlishipping/rule')
          		...
          		->addWeightFilter($this->getWeight())
          		->getFirstItem();
             //echo "<pre>"; print_r($rule->getSelect()); exit;
          	if($rule && $rule->getId()){
          		$amount = $rule->getAmount();
          	}
        }
	...
	public function getProductCollection( $filter = false  , $pageNumber = NULL ){
		...
		$collection->addFieldToFilter ('entity_id',5851);
