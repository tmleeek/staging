...
/**
 * created : 2 oct. 2009
 * System config model for file
 *
 * updated by <user> : <date>
 *
 *
 * @category SQLI
 * @package Sqli_Adminhtml
 * @author ysanchez
 ...
 * @package Sqli_Adminhtml
 */
class Tatva_TransferPayment_Model_System_Config_Backend_File extends Mage_Core_Model_Config_Data
{
	/**
	...
	protected function _beforeSave() {
		...
		if ($_FILES['groups']['tmp_name'][$this->getGroupId()]['fields'][$this->getField()]['value']){
			...
			if (empty($fieldConfig->upload_dir)) {
				Mage::throwException(Mage::helper('catalog')->__('Base directory to upload file is not specified'));
			}
