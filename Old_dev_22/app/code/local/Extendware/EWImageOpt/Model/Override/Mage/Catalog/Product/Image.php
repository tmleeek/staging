...
class Extendware_EWImageOpt_Model_Override_Mage_Catalog_Product_Image extends Extendware_EWImageOpt_Model_Override_Mage_Catalog_Product_Image_Bridge
{
	...
	static protected function canCacheImages() {
		$canCacheImages = Mage::helper('ewimageopt/config')->isCatalogImageCacheEnabled();
		if ($canCacheImages === true) {
			$canCacheImages = Mage::helper('ewimageopt')->getCanCacheImages();
		}
		return $canCacheImages;
	}
        ...
        if (self::canCacheImages() === true) {
	        ...
	        if (Mage::helper('ewimageopt/config')->isCatalogImageSeoEnabled()) {
		        ...
		        if ($product and $product->getName()) {
		        	$filename = Mage::helper('ewimageopt')->toSlug(strtolower($product->getName()));
