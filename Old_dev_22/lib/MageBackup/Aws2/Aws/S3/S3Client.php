...
use MageBackup\Aws\Common\Client\AbstractClient;
use MageBackup\Aws\Common\Client\ClientBuilder;
use MageBackup\Aws\Common\Client\ExpiredCredentialsChecker;
use MageBackup\Aws\Common\Client\UploadBodyListener;
use MageBackup\Aws\Common\Enum\ClientOptions as Options;
use MageBackup\Aws\Common\Exception\RuntimeException;
use MageBackup\Aws\Common\Exception\InvalidArgumentException;
use MageBackup\Aws\Common\Signature\SignatureV4;
use MageBackup\Aws\Common\Model\MultipartUpload\AbstractTransfer;
use MageBackup\Aws\S3\Exception\AccessDeniedException;
use MageBackup\Aws\S3\Exception\Parser\S3ExceptionParser;
use MageBackup\Aws\S3\Exception\S3Exception;
use MageBackup\Aws\S3\Model\ClearBucket;
use MageBackup\Aws\S3\Model\MultipartUpload\AbstractTransfer as AbstractMulti;
use MageBackup\Aws\S3\Model\MultipartUpload\UploadBuilder;
use MageBackup\Aws\S3\Sync\DownloadSyncBuilder;
use MageBackup\Aws\S3\Sync\UploadSyncBuilder;
use MageBackup\Guzzle\Common\Collection;
use MageBackup\Guzzle\Http\EntityBody;
use MageBackup\Guzzle\Http\Message\RequestInterface;
use MageBackup\Guzzle\Iterator\FilterIterator;
use MageBackup\Guzzle\Plugin\Backoff\BackoffPlugin;
use MageBackup\Guzzle\Plugin\Backoff\CurlBackoffStrategy;
use MageBackup\Guzzle\Plugin\Backoff\ExponentialBackoffStrategy;
use MageBackup\Guzzle\Plugin\Backoff\HttpBackoffStrategy;
use MageBackup\Guzzle\Plugin\Backoff\TruncatedBackoffStrategy;
use MageBackup\Guzzle\Service\Command\CommandInterface;
use MageBackup\Guzzle\Service\Command\Factory\AliasFactory;
use MageBackup\Guzzle\Service\Command\Factory\CompositeFactory;
use MageBackup\Guzzle\Service\Resource\Model;
use MageBackup\Guzzle\Service\Resource\ResourceIteratorInterface;
...
class S3Client extends AbstractClient
{
    ...
    public static function factory($config = array())
    {
        ...
        $default->add(
            ...
            'MageBackup\Guzzle\Service\Command\Factory\ServiceDescriptionFactory'
        );
     ...
     * @param $config
     *
     * @return \MageBackup\Aws\Common\Signature\SignatureInterface
     ...
     * @param mixed  $body    Object data to upload. Can be a MageBackup\Guzzle\Http\EntityBodyInterface, stream resource, or
     ...
     *       MageBackup\Guzzle\Common\Event object with context.
     *
     * @see MageBackup\Aws\S3\Model\MultipartUpload\UploadBuilder for more options and customization
     * @return \MageBackup\Guzzle\Service\Resource\Model Returns the modeled result of the performed operation
     */
     ...
     *       multipart upload.
     *
     * @see MageBackup\Aws\S3\S3Sync\S3Sync for more options and customization
     */
     ...
     *       MageBackup\Guzzle\Common\Event object with context.
     *
     * @see MageBackup\Aws\S3\S3Client::listObjects
     * @see MageBackup\Aws\S3\Model\ClearBucket For more options or customization
     * @return int Returns the number of deleted keys
