...
use MageBackup\Guzzle\Http\RedirectPlugin;
...
class RequestFactory implements RequestFactoryInterface
{
    ...
    protected $requestClass = 'MageBackup\Guzzle\\Http\\Message\\Request';

    /** @var string Class to instantiate for requests with a body */
    protected $entityEnclosingRequestClass = 'MageBackup\Guzzle\\Http\\Message\\EntityEnclosingRequest';
     ...
     * {@see MageBackup\Guzzle\Http\Message\Request::clone}, but can change the HTTP method.
     *
    ...
    protected function visit_exceptions(RequestInterface $request, $value, $flags)
    {
        if ($value === false || $value === 0) {
            ...
            foreach ($dispatcher->getListeners('request.error') as $listener) {
                if (is_array($listener) && $listener[0] == 'MageBackup\Guzzle\Http\Message\Request' && $listener[1] = 'onRequestError') {
