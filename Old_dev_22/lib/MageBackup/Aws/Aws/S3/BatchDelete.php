...
use MageBackup\Aws\S3\Exception\DeleteMultipleObjectsException;
 ...
 * operation.
 *
 *     $s3 = new MageBackup\Aws\S3\Client([
 *         'region' => 'us-west-2',
 *         'version' => 'latest'
 *     ]);
 *
 *     $listObjectsParams = ['Bucket' => 'foo', 'Prefix' => 'starts/with/'];
 *     $delete = MageBackup\Aws\S3\BatchDelete::fromListObjects($s3, $listObjectsParams);
...
class BatchDelete implements PromisorInterface
{
    ...
    public static function fromIterator(
        ...
        $fn = function (BatchDelete $that) use ($iter) {
            return \MageBackup\GuzzleHttp\Promise\coroutine(function () use ($that, $iter) {
                foreach ($iter as $obj) {
