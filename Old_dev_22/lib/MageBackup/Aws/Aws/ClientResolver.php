...
use MageBackup\Aws\Api\Validator;
...
use MageBackup\Aws\Endpoint\Partition;
use MageBackup\Aws\Endpoint\PartitionEndpointProvider;
use MageBackup\Aws\Endpoint\PartitionProviderInterface;
use MageBackup\Aws\Signature\SignatureProvider;
use MageBackup\Aws\Endpoint\EndpointProvider;
use MageBackup\Aws\Credentials\CredentialProvider;
...
class ClientResolver
{
    ...
    private static $defaultArgs = [
        'service' => [
            'type'     => 'value',
            'valid'    => ['string'],
            'doc'      => 'Name of the service to utilize. This value will be supplied by default when using one of the SDK clients (e.g., MageBackup\Aws\\S3\\S3Client).',
            'required' => true,
            'internal' => true
        ],
        'exception_class' => [
            ...
            'default'  => 'MageBackup\Aws\Exception\AwsException',
            'internal' => true
        ],
        'scheme' => [
            'type'     => 'value',
        ...
        'signature_provider' => [
            'type'    => 'value',
            'valid'   => ['callable'],
            'doc'     => 'A callable that accepts a signature version name (e.g., "v4"), a service name, and region, and  returns a SignatureInterface object or null. This provider is used to create signers utilized by the client. See MageBackup\Aws\\Signature\\SignatureProvider for a list of built-in providers',
        ...
        'endpoint_provider' => [
            ...
            'doc'      => 'An optional PHP callable that accepts a hash of options including a "service" and "region" key and returns NULL or a hash of endpoint data, of which the "endpoint" key is required. See MageBackup\Aws\\Endpoint\\EndpointProvider for a list of built-in providers.',
        ...
        'credentials' => [
            ...
            'doc'     => 'Specifies the credentials used to sign requests. Provide an MageBackup\Aws\Credentials\CredentialsInterface object, an associative array of "key", "secret", and an optional "token" key, `false` to use null credentials, or a callable credentials provider used to create credentials or return null. See MageBackup\Aws\\Credentials\\CredentialProvider for a list of built-in credentials providers. If no credentials are provided, the SDK will attempt to load them from the environment.',
        ...
        'stats' => [
            ...
            'valid' => ['bool', 'array'],
            'default' => false,
            'doc'   => 'Set to true to gather transfer statistics on requests sent. Alternatively, you can provide an associative array with the following keys: retries: (bool) Set to false to disable reporting on retries attempted; http: (bool) Set to true to enable collecting statistics from lower level HTTP adapters (e.g., values returned in MageBackup\GuzzleHttp\TransferStats). HTTP handlers must support an http_stats_receiver option for this to have an effect; timer: (bool) Set to true to enable a command timer that reports the total wall clock time spent on an operation in seconds.',
        ...
        'handler' => [
            'type'     => 'value',
            'valid'    => ['callable'],
            'doc'      => 'A handler that accepts a command object, request object and returns a promise that is fulfilled with an MageBackup\Aws\ResultInterface object or rejected with an MageBackup\Aws\Exception\AwsException. A handler does not accept a next handler as it is terminal and expected to fulfill a command. If no handler is provided, a default Guzzle handler will be utilized.',
    ...
    public static function _apply_credentials($value, array &$args)
    {
        if (is_callable($value)) {
            ...
            throw new IAE('Credentials must be an instance of '
                . 'MageBackup\Aws\Credentials\CredentialsInterface, an associative '
